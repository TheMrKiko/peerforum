...
class rating_manager {
    ...
    /**
     * Returns an array of ratings for a given item (forum post, glossary entry etc). This returns all users ratings for a single
     * item
     *
     * @param stdClass $options {
     ...
     *         RATING_AGGREGATE_AVERAGE, RATING_AGGREGATE_MAXIMUM etc [required] scaleid          => int the scale from which the
     *         user can select a rating [required] userid           => int the id of the current user [optional] returnurl
     *         => string the url to return the user to after submitting a rating. Can be left null for ajax requests [optional]
     *         assesstimestart  => int only allow rating of items created after this timestamp [optional] assesstimefinish => int
     *         only allow rating of items created before this timestamp [optional]
    ...
    public function get_ratings($options) {
        ...
        list($itemtablename, $itemidcol, $itemuseridcol) =
                plugin_callback($type, $name, 'rating', 'get_item_fields', array($options), $default);
     ...
     *      aggregate         => int what aggregation method should be applied. RATING_AGGREGATE_AVERAGE, RATING_AGGREGATE_MAXIMUM
     *         etc [required] scaleid           => int the scale from which the user can select a rating [required] returnurl
     *           => string the url to return the user to after submitting a rating. Can be left null for ajax requests [optional]
     *         assesstimestart   => int only allow rating of items created after this timestamp [optional] assesstimefinish  => int
     *         only allow rating of items created before this timestamp [optional] plugintype        => string plugin type ie 'mod'
     *         Used to find the permissions callback [optional] pluginname        => string plugin name ie 'forum' Used to find the
     *         permissions callback [optional]
     * }
    ...
    protected function generate_rating_settings_object($options) {
        ...
        $settings->permissions->view =
                has_capability('moodle/rating:view', $options->context); // can view the aggregate of ratings of their own items
        $settings->permissions->viewany = has_capability('moodle/rating:viewany',
                $options->context); // can view the aggregate of ratings of other people's items
        ...
        $pluginpermissionsarray =
                $this->get_plugin_permissions_array($options->context->id, $options->component, $options->ratingarea);
     ...
     *
     * @param stdClass $options {
     *            userid => int the id of the user whose items have been rated. NOT the user who submitted the ratings. 0 to update
     *         all. [required] aggregationmethod => int the aggregation method to apply when calculating grades ie
     *         RATING_AGGREGATE_AVERAGE [required] scaleid => int the scale from which the user can select a rating. Used for
     *         bounds checking. [required] itemtable => int the table containing the items [required] itemtableusercolum => int the
     *         column of the user table containing the item owner's user id [required] component => The component for the ratings
     *         [required] ratingarea => The ratingarea for the ratings [required] contextid => int the context in which the rated
     *         items exist [optional] modulename => string the name of the module [optional] moduleid => int the id of the module
     *         instance [optional]
     * }
    ...
    public function get_plugin_permissions_array($contextid, $component, $ratingarea) {
        ...
        $defaultpluginpermissions =
                array('rate' => false, 'view' => false, 'viewany' => false, 'viewall' => false);//deny by default
        if (!empty($component)) {
            list($type, $name) = core_component::normalize_component($component);
            $pluginpermissionsarray =
                    plugin_callback($type, $name, 'rating', 'permissions', array($contextid, $component, $ratingarea),
                            $defaultpluginpermissions);
        } else {
     ...
     * @return boolean true if the rating is valid. False if callback wasnt found and will throw rating_exception if rating is
     *         invalid
     */
...
/**
 * The rating_exception class provides the ability to generate exceptions that can be easily identified as coming from the ratings
 * system
 *
 * @package   core_rating
 * @category  rating
