...
$url = new moodle_url('/peergrading/test.php');

$urlparams = compact('userid', 'courseid', 'display');

$PAGE->set_url('/peergrading/test.php',
...
if ($mform->is_cancelled()) {
    ...
    if (!empty($resultsfav = $fromform->listoffavstudents) && !empty($resultsunfav = $fromform->listofleastfavstudents)) {

        //Verify if student isnt on table already
        $studenthasdonequest = student_answered_questionnaire($userid);
        //Verify if student has 9 different answers
