...
if (!empty($postinfo)) {
    ...
    foreach ($conflit as $key => $value) {
        ...
        if (in_array($postauthor, $conflit_stds) && in_array($user, $conflit_stds)) {
            $erro =
                    $OUTPUT->notification("There's a conflit between post author $author_name and  $user_name. Post cannot be assigned",
                            'notifyproblem');
            $returnurl = new moodle_url('/peergrading/index.php',
                    array('userid' => $userid, 'courseid' => $courseid, 'display' => $display));
            redirect($returnurl, $erro, 10);
    ...
    $returnurl =
            new moodle_url('/peergrading/index.php', array('userid' => $userid, 'courseid' => $courseid, 'display' => $display));
    redirect($returnurl, $erro, 10);
