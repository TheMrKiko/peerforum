...
require_once($CFG->dirroot . '/peergrading/settings.php');
...
//Before starting to load this page
...
if (has_capability('mod/peerforum:viewpanelpeergrades', $context)) {
    ...
    if ($display == '-1' || $display == '1') {
        ...
        $display = '6';
        $currenttab = 'display_managetraining';
    ...
    //Optional tabs
    if ($training) {
        $row[] = new tabobject('display_managetraining',
...
if ($display == '-1') {
    if (has_capability('mod/peerforum:viewpanelpeergrades', $context)) {
        ...
        if (!empty($posts)) {
            ...
            //EDIT HERE
            $all_students = $DB->get_records("peerforum_peergrade_users", array('peergradetype' => 1));
            foreach ($posts as $i => $value) {
                ...
                $post_author_id = $posts[$i]->authorid; //userid?
                ...
                //order peergraders by name
                uasort($posts[$i]->peergraders, function($a, $b) {
                    return strcmp($a->authorname, $b->authorname);
                });

                foreach ($posts[$i]->peergraders as $key => $value) { //when it has already attributed students to peer grade
            ...
            foreach ($infograder as $i => $value) {
                ...
                if (!has_capability('mod/peerforum:viewallpeergrades', $PAGE->context, $i, false)) {
                    ...
                    foreach ($infograder[$i]->posts as $key => $value) {
                        ...
                        $post_time = $DB->get_record('peerforum_posts', array('id' => $postid))->created;
                        $time = userdate($post_time, '%e/%m/%y');
            ...
            if (!in_array($userposts[$i], $userposts_graded)) {
                ...
                if (!empty($post)) {
                    ...
                    if ($training) {
                        ...
                        //Link to check examples on how to grade
                        $topic_name = ($DB->get_record("peerforum_discussions", array('id' => $post->discussion)))->name;
...
if ($display == '6') {
    ...
    if (has_capability('mod/peerforum:viewpanelpeergrades', $context)) {
        ...
        $info_subjects = $DB->get_records("peerforum_discussions", array('course' => $courseid, 'peerforum' => $pf->id));

        if (empty($info_subjects)) {
            echo 'No information to display.';
        } else {
            echo '<table id="TrainingStatsTable" class="managepeers"">' .
                    '<tr">' .
                    '<th bgcolor=#cccccc> Name </th>' .
                    '<th bgcolor=#cccccc> Link ID </th>' .
                    '<th bgcolor=#cccccc> Configure </th>' .
                    '</tr>';

            $even = true;

            //List all subjects
            foreach ($info_subjects as $id => $value) {
                ...
                //Name Row
                $name = $info_subjects[$id]->name;
                $name_link = html_writer::link(new moodle_url('/peergrading/training/' . $name . '.html'), $name);

                //Edit row
                $link_id = $info_subjects[$id]->idlink;

                if (empty($link_id)) {
                    $addurl = new moodle_url('/peergrading/add_idlink.php',
                            array('userid' => $userid, 'courseid' => $courseid, 'display' => $display,
                                    'id' => $info_subjects[$id]->id));

                    $link_id = " <form action='$addurl' method='post'><br>
                                <input type='number' id='idvalue' name='idvalue'>
                                <input type='submit' id='submitidvalue' name='submitidvalue' value='Submit' >
                              </form>";
                }

                $link_url = new moodle_url('/course/modedit.php', array('update' => $link_id, 'return' => 0, 'sr' => 0));
                $config_button = '<form action="' . $link_url . '" method="post">' .
                        '<div class="buttons"><input style="margin-left:3px;margin-top:1px; margin-bottom:1px;font-size:13px;" type="submit" id="block" name="block" value="' .
                        get_string('edit', 'block_peerblock') . '"/>' .
                        '</div></form>';

                echo '<tr>' . '<td width="150" bgcolor=' . "$color" . '>' . $name_link . '</td>' . '<td width="50" bgcolor=' .
                        "$color" . '>' . $link_id . '</td>' . '<td width="50" bgcolor=' . "$color" . '>' . $config_button .
                        '</td>' . '</tr>';
        ...
        if (!empty($info_peers)) {
            ...
            foreach ($info_peers as $key => $value) {
                ...
                echo '<tr>' . '<td width="150" bgcolor=' . "$color" . '>' . $grader_link . '</td>' . '<td width="50" bgcolor=' .
                        ...
                        "$color" . '>' . $numexpired . '</td>' . '<td width="50" bgcolor=' . "$color" . '>' . $userblocked .
                        '</td>' . '</tr>';
            }
...
if ($display == '8') {
    if (has_capability('mod/peerforum:viewpanelpeergrades', $context)) {
        ...
        $info_peers = $DB->get_records('peerforum_relationships', array('courseid' => $courseid));
        $info_peers = order_students_by_name($info_peers);
        ...
        if (!empty($info_peers)) {
            ...
            if ($managerelationsmode == RELATIONSHIPS_MODE_NOMINATIONS) {
                echo '<table class="managepeers">' .
                        '<tr">' .
                        '<th bgcolor=#cccccc> Student </th>' .
                        '<th bgcolor=#cccccc> Favorite Students </th>' .
                        '<th bgcolor=#cccccc> Edit </th>' .
                        '<th bgcolor=#cccccc> Least Favorite Students </th>' .
                        '<th bgcolor=#cccccc> Edit </th>' .
                        '</tr>';
                ...
                foreach ($info_peers as $i => $value) {
                    ...
                    //Decide how many rows needed
                    if ((count($favstudents)) > (count($leastfavstudents))) {
                        $numentries = count($favstudents) + 1;
                    } else {
                        $numentries = count($leastfavstudents) + 1;
                    }
...
if ($display == '9') {
    if (has_capability('mod/peerforum:viewpanelpeergrades', $context)) {
        ...
        $info_discussions = $DB->get_records("peerforum_discussions", array('course' => $courseid, 'peerforum' => $pf->id));
        ...
        if (!empty($info_discussions)) {
            ...
            $students_eligible =
                    $DB->get_records('peerforum_peergrade_users', array('courseid' => $courseid, 'peergradetype' => 1));
            ...
            foreach ($info_discussions as $key => $value) {
                ...
                if ($pf->random_distribution) {
                    $count = (count($students_eligible)) / (count($info_discussions)) + 1;
                ...
                if ($discussion_type == 1) {
                    foreach ($students_eligible as $i => $value) {
                        ...
                        if (in_array($info_discussions[$key]->name, $topics)) {
                            ...
                            //alert if student is Blocked
                            if ($students_eligible[$i]->userblocked) {
                                $color = '#ffb3b3';
                            }
...
// Peers to Rank
if ($display == '10') {
    echo $OUTPUT->heading(format_string($peerranking), 2);
