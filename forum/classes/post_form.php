...
class mod_forum_post_form extends moodleform {
    ...
    function definition() {
        ...
        if (isset($forum->id) && forum_is_forcesubscribed($forum)) {
            ...
            $mform->addElement('static', 'subscribemessage', get_string('subscription', 'forum'),
            ...
            $mform->addElement('static', 'subscribemessage', get_string('subscription', 'forum'),
                    get_string('disallowsubscribe', 'forum'));
            $mform->addElement('hidden', 'subscribe');
            $mform->setType('subscribe', PARAM_INT);
            $mform->addHelpButton('subscribemessage', 'subscription', 'forum');
        }
        ...
        if (!empty($forum->maxattachments) && $forum->maxbytes != 1 &&
                has_capability('mod/forum:createattachment', $modcontext)) {  //  1 = No attachments at all
