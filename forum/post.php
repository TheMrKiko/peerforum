...
if (!isloggedin() or isguestuser()) {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->confirm(get_string('noguestpost', 'forum') . '<br /><br />' . get_string('liketologin'), get_login_url(),
            $referer);
    echo $OUTPUT->footer();
    exit;
}
...
if (!empty($forum)) {      // User is starting a new discussion in a forum
    ...
    if ($prunemform->is_cancelled()) {
        ...
        $PAGE->navbar->add(format_string($post->subject, true),
                new moodle_url('/mod/forum/discuss.php', array('d' => $discussion->id)));
...
$currenttext = file_prepare_draft_area($draftid_editor, $modcontext->id, 'mod_forum', 'post', $postid,
        mod_forum_post_form::editor_options($modcontext, $postid), $post->message);
...
if ($mform_post->is_cancelled()) {
    ...
    if ($fromform->edit) {           // Updating a post
        ...
        // MDL-11818
        if (($forum->type == 'single') &&
                ($updatepost->parent == '0')) { // updating first post of single discussion type -> updating forum intro
