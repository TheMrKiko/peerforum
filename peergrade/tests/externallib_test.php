...
class core_peergrade_externallib_testcase extends externallib_advanced_testcase {
    ...
    public function setUp(): void {
        ...
        $record->scale = 100;
        $this->peerforum = self::getDataGenerator()->create_module('peerforum', $record);

        $this->contextid = context_module::instance($this->peerforum->cmid)->id;

        // Add discussion to the forums.
        $record = new stdClass();
        $record->course = $this->course->id;
        $record->userid = $this->student1->id;
        $record->peerforum = $this->peerforum->id;
        $this->discussion = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record);
        // Retrieve the first post.
        $this->post = $DB->get_record('peerforum_posts', array('discussion' => $this->discussion->id));
    }
    ...
    public function test_get_item_peergrades() {
        ...
        $peergrade1->component = 'mod_peerforum';
        ...
        $peergrade2->component = 'mod_peerforum';
        ...
        $peergrades =
                core_peergrade_external::get_item_peergrades('module', $this->peerforum->cmid, 'mod_peerforum', 'post', $this->post->id,
                        100, '');
        ...
        try {
            $peergrades =
                    core_peergrade_external::get_item_peergrades('module', $this->peerforum->cmid, 'mod_peerforum', 'post', 0, 100, '');
        ...
        try {
            $peergrades =
                    core_peergrade_external::get_item_peergrades('module', $this->peerforum->cmid, 'mod_peerforum', 'xyz', $this->post->id,
                            100,
                            '');
        ...
        try {
            $peergrades = core_peergrade_external::get_item_peergrades('module', 0, 'mod_peerforum', 'post', $this->post->id, 100, '');
        ...
        // Test for groupmode.
        set_coursemodule_groupmode($this->peerforum->cmid, SEPARATEGROUPS);
        ...
        $DB->update_record('peerforum_discussions', $this->discussion);
    ...
    public function test_add_peergrade() {
        ...
        $peergrade = core_peergrade_external::add_peergrade('module', $this->peerforum->cmid, 'mod_peerforum', 'post', $this->post->id, 100,
                50, $this->student1->id, PEERGRADE_AGGREGATE_AVERAGE, 'Nice.');
        // We need to execute the return values cleaning process to simulate the web service server.
        ...
        $peergrade = core_peergrade_external::add_peergrade('module', $this->peerforum->cmid, 'mod_peerforum', 'post', $this->post->id, 100,
                100, $this->student1->id, PEERGRADE_AGGREGATE_AVERAGE, 'Meh.');
        $peergrade = external_api::clean_returnvalue(core_peergrade_external::add_peergrade_returns(), $peergrade);
        $this->assertTrue($peergrade['success']);
        ...
        $peergrade = core_peergrade_external::add_peergrade('module', $this->peerforum->cmid, 'mod_peerforum', 'post', $this->post->id, 100,
                100, $this->student1->id, PEERGRADE_AGGREGATE_AVERAGE, 'Nice.');
    }
}
