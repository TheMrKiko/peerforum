...
if (!empty($postparent)) {
    ...
    foreach ($peers as $key => $value) {
        ...
        array_push($peernames,
                html_writer::tag('span', $peer_name, array('id' => 'peersassigned' . $itemid, 'style' => 'color:' . $color . ';')));
    ...
    $students_html = html_writer::select($studentsarray, 'menuassignpeer' . $itemid, $studentsarray[UNSET_STUDENT_SELECT], false,
            $studentattrs);
...
echo json_encode(array('result' => $result, 'itemid' => $itemid, 'peers' => $peers, 'peersnames' => $peersnames,
        'canassign' => $students_html, 'canremove' => $students_rmv_html));
