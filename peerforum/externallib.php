...
class mod_peerforum_external extends external_api {
     ...
     * @see get_peerforum_discussions_paginated
     */
     ...
     * @see get_peerforum_discussions_paginated
     */
    public static function get_peerforum_discussions($peerforumids, $limitfrom = 0, $limitnum = 0) {
        global $CFG, $DB, $USER;
    ...
    public static function get_peerforum_discussion_posts($discussionid, $sortby = "created", $sortdirection = "DESC") {
        ...
        foreach ($allposts as $post) {
            ...
            if (!empty($post->attachment)) {
                ...
                $fs = get_file_storage();
                if ($files = $fs->get_area_files($modcontext->id, 'mod_peerforum', 'attachment', $post->id, "filename", false)) {
                    foreach ($files as $file) {
                        $filename = $file->get_filename();
                        $fileurl = moodle_url::make_webservice_pluginfile_url(
                                $modcontext->id, 'mod_peerforum', 'attachment', $post->id, '/', $filename);
    ...
    public static function get_peerforum_discussions_paginated($peerforumid, $sortby = 'timemodified', $sortdirection = 'DESC',
        ...
        if ($alldiscussions) {
            ...
            // The peerforum function returns the replies for all the discussions in a given peerforum.
            $replies = peerforum_count_discussion_replies($peerforumid, $sort, -1, $page, $perpage);
            ...
            foreach ($alldiscussions as $discussion) {
                ...
                list($discussion->message, $discussion->messageformat) =
                        external_format_text($discussion->message, $discussion->messageformat,
                                $modcontext->id, 'mod_peerforum', 'post', $discussion->id);
                ...
                if (!empty($discussion->attachment)) {
                    ...
                    $fs = get_file_storage();
                    if ($files = $fs->get_area_files($modcontext->id, 'mod_peerforum', 'attachment',
                        ...
                        foreach ($files as $file) {
                            ...
                            $discussion->attachments[] = array(
                                    ...
                                    'fileurl' => file_encode_url($CFG->wwwroot . '/webservice/pluginfile.php',
                                            '/' . $modcontext->id . '/mod_peerforum/attachment/' . $discussion->id . '/' .
                                            $filename)
                            );
                        }
                    }
                        ...
                        'discussions' => new external_multiple_structure(
                                new external_single_structure(
                                        array(
                                                ...
                                                'numreplies' => new external_value(PARAM_TEXT,
    ...
    public static function add_discussion_post($postid, $subject, $message, $options = array()) {
        ...
        if (!peerforum_user_can_post($peerforum, $discussion, $USER, $cm, $course, $context)) {
        ...
        if ($postid = peerforum_add_new_post($post, null)) {
     ...
     * @since Moodle 3.0
     */
    public static function add_discussion($peerforumid, $subject, $message, $groupid = -1, $options = array()) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/peerforum/lib.php");
        ...
        if ($discussionid = peerforum_add_discussion($discussion)) {
