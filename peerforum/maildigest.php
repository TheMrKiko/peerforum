...
/**
 * Set the mail digest option in a specific peerforum for a user.
 *
...
$peerforum = $DB->get_record('peerforum', array('id' => $id));
...
$url = new moodle_url('/mod/peerforum/maildigest.php', array(
        'id' => $id,
...
peerforum_set_user_maildigest($peerforum, $maildigest);
...
if ($maildigest === -1) {
    ...
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
            'mod_peerforum', $info);
    $updatemessage = get_string('emaildigestupdated_default', 'peerforum', $info);
} else {
    $info->maildigesttitle = $digestoptions[$info->maildigest];
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
            'mod_peerforum', $info);
    $updatemessage = get_string('emaildigestupdated', 'peerforum', $info);
}
