...
/**
 * Page to export peerforum discussions.
 *
...
$peerforumid = required_param('id', PARAM_INT);
...
if (!$capabilitymanager->can_export_peerforum($USER)) {
    throw new moodle_exception('cannotexportpeerforum', 'peerforum');
}
...
$url = new moodle_url('/mod/peerforum/export.php');
$pagetitle = get_string('export', 'mod_peerforum');
$context = $peerforum->get_context();

$form = new mod_peerforum\form\export_form($url->out(false), [
        'peerforum' => $peerforum
]);

if ($form->is_cancelled()) {
    redirect(new moodle_url('/mod/peerforum/view.php', ['id' => $cm->id]));
    ...
    if ($data->discussionids) {
        ...
        $discussions = $discussionvault->get_all_discussions_in_peerforum($peerforum);
    ...
    \core\dataformat::download_data(
            ...
            function($exportdata) use ($fields, $striphtml, $humandates, $canviewfullname, $context) {
                ...
                foreach ($fields as $field) {
                    ...
                    if ($field == 'message') {
                        $data->message =
                                file_rewrite_pluginfile_urls($data->message, 'pluginfile.php', $context->id, 'mod_peerforum',
                                        'post', $data->id);
                    }
