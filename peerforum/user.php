...
$isparent =
        !$iscurrentuser && $DB->record_exists('role_assignments', array('userid' => $USER->id, 'contextid' => $usercontext->id));
...
if (empty($result->posts)) {
    ...
    if ($iscurrentuser) {
        ...
        // These are the user's peerforum interactions.
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'peerforum'), new moodle_url('/mod/peerforum/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'peerforum'), new moodle_url('/mod/peerforum/user.php',
...
$postsrenderer = $rendererfactory->get_user_peerforum_posts_report_renderer(!$isspecificcourse && !$hasparentaccess);
$postoutput = $postsrenderer->render(
        $USER,
        array_map(function($peerforum) use ($entityfactory, $result) {
            $cm = $peerforum->cm;
            $context = context_module::instance($cm->id);
            $course = $result->courses[$peerforum->course];
            return $entityfactory->get_peerforum_from_stdclass($peerforum, $context, $cm, $course);
        }, $result->peerforums),
