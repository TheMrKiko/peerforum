 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peerforum_restore_date_testcase extends restore_date_testcase {
    ...
    public function test_restore_dates() {
        ...
        $gg = $this->getDataGenerator()->get_plugin_generator('mod_peerforum');
        $record = ['assesstimefinish' => 100, 'assesstimestart' => 100, 'ratingtime' => 1, 'assessed' => 2, 'scale' => 1];
        list($course, $peerforum) = $this->create_course_and_module('peerforum', $record);

        // PeerForum Discussions/posts/ratings.
        $timestamp = 996699;
        $diff = $this->get_diff();
        $record = new stdClass();
        ...
        $DB->set_field('peerforum_discussions', 'timemodified', $timestamp);

        // Ratings.
        $ratingoptions = new stdClass;
        $ratingoptions->context = context_module::instance($peerforum->cmid);
        ...
        $newpeerforum = $DB->get_record('peerforum', ['course' => $newcourseid]);

        $this->assertFieldsNotRolledForward($peerforum, $newpeerforum, ['timemodified']);
        $props = ['assesstimefinish', 'assesstimestart'];
        $this->assertFieldsRolledForward($peerforum, $newpeerforum, $props);

        $newdiscussion = $DB->get_record('peerforum_discussions', ['peerforum' => $newpeerforum->id]);
        $newposts = $DB->get_records('peerforum_posts', ['discussion' => $newdiscussion->id]);
        $newcm = $DB->get_record('course_modules', ['course' => $newcourseid, 'instance' => $newpeerforum->id]);

        // PeerForum discussion time checks.
