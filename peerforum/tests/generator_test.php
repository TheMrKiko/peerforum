...
class mod_peerforum_generator_testcase extends advanced_testcase {
    ...
    public function test_create_discussion() {
        ...
        $record['pinned'] = PEERFORUM_DISCUSSION_PINNED; // Pin one discussion.
        self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record);
        $record['pinned'] = PEERFORUM_DISCUSSION_UNPINNED; // No pin for others.
        self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record);
        self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record);
        ...
        $record = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record);
        $this->assertEquals(array('Cats', 'mice'),
                array_values(core_tag_tag::get_item_tags_array('mod_peerforum', 'peerforum_posts', $record->firstpost)));
    }
    ...
    public function test_create_post() {
        ...
        $record->tags = array('Cats', 'mice');
        $record = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_post($record);
        $this->assertEquals(array('Cats', 'mice'),
                array_values(core_tag_tag::get_item_tags_array('mod_peerforum', 'peerforum_posts', $record->id)));
    }
    ...
    public function test_create_content() {
        ...
        $post5 = $generator->create_content($peerforum, array('parent' => $post2->id, 'tags' => array('Cats', 'mice')));
        ...
        $this->assertEquals(array('Cats', 'mice'),
                array_values(core_tag_tag::get_item_tags_array('mod_peerforum', 'peerforum_posts', $post5->id)));
    }
}
