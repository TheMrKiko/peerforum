...
/**
 * The peerforum module cron trait.
 *
...
trait mod_peerforum_tests_cron_trait {
    /**
    ...
    protected function queue_tasks_and_assert($expectations = []) {
        ...
        ob_start();
        cron_setup_user();
        $cron = new \mod_peerforum\task\cron_task();
        $cron->execute();
        ...
        // Update the peerforum queue for digests.
        $DB->execute("UPDATE {peerforum_queue} SET timemodified = timemodified - 1");
    }
    ...
    protected function send_notifications_and_assert($user, $posts = [], $ignoreemptyposts = false) {
        ob_start();
        $this->runAdhocTasks(\mod_peerforum\task\send_user_notifications::class, $user->id);
        $output = ob_get_contents();
        ob_end_clean();
    ...
    protected function send_digests_and_assert($user, $fullposts = [], $shortposts = []) {
        ob_start();
        $this->runAdhocTasks(\mod_peerforum\task\send_user_digests::class, $user->id);
        $output = ob_get_contents();
        ob_end_clean();
        ...
        if (empty($shortposts) && empty($fullposts)) {
            ...
            $this->assertRegExp("/Sending peerforum digests for {$user->username}/", $output);
