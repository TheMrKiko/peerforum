...
class mod_peerforum_maildigest_testcase extends advanced_testcase {
    ...
    public function test_cron_profile_single_mails() {
        ...
        $posts = [];
        for ($i = 0; $i < 5; $i++) {
            list($discussion, $post) = $this->helper_post_to_peerforum($peerforum1, $user, ['mailnow' => 1]);
            $posts[] = $post;
        }

        // Add 5 discussions to peerforum 2.
        for ($i = 0; $i < 5; $i++) {
            list($discussion, $post) = $this->helper_post_to_peerforum($peerforum2, $user, ['mailnow' => 1]);
            $posts[] = $post;
        }
    ...
    public function test_cron_digest_view_fullnames_off() {
        ...
        // Add 1 discussions to peerforum 1.
        list($discussion, $post) = $this->helper_post_to_peerforum($peerforum1, $user, ['mailnow' => 1]);
        $posts[] = $post;
        ...
        // No digests mails should be sent, but 1 peerforum mails will be sent.
        $expect = [
                (object) [
                        'userid' => $user->id,
                        'messages' => 0,
    ...
    public function test_cron_digest_marks_posts_read() {
        ...
        // There should be unread posts for the peerforum.
        $expectedposts = [
                $peerforum1->id => (object) [
                        'id' => $peerforum1->id,
                        'unread' => count($posts),
                ],
        ];
        $this->assertEquals($expectedposts, peerforum_tp_get_course_unread_posts($user->id, $course1->id));
        ...
        // Verify that there are no unread posts for any peerforums.
        $this->assertEmpty(peerforum_tp_get_course_unread_posts($user->id, $course1->id));
    }
    ...
    public function test_cron_digest_leaves_posts_unread() {
        ...
        // Verify that there are still the same unread posts for the peerforum.
        $this->assertEquals($expectedposts, peerforum_tp_get_course_unread_posts($user->id, $course1->id));
    }
}
