 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peerforum_vaults_discussion_list_testcase extends advanced_testcase {
    // Make use of the test generator trait.
    use mod_peerforum_tests_generator_trait;

    /** @var \mod_peerforum\local\vaults\discussion_list */
    private $vault;
    ...
    public function setUp(): void {
        ...
        $this->vault = $vaultfactory->get_discussions_in_peerforum_vault();
    }
    ...
    /**
     * Test get_from_peerforum_id.
     */
    public function test_get_from_peerforum_id() {
        $this->resetAfterTest();
        ...
        $this->assertEquals([], $vault->get_from_peerforum_id($peerforum->id, true, $user->id, null,
                0, 0));

        $now = time();
        [$discussion1, $post1] =
                $this->helper_post_to_peerforum($peerforum, $user, ['timestart' => $now - 10, 'timemodified' => 1]);
        [$discussion2, $post2] = $this->helper_post_to_peerforum($peerforum, $user, ['timestart' => $now - 9, 'timemodified' => 2]);
        [$hiddendiscussion, $post3] =
                $this->helper_post_to_peerforum($peerforum, $user, ['timestart' => $now + 10, 'timemodified' => 3]);
        [$discussion3, $post4] = $this->helper_post_to_peerforum($peerforum, $user, ['timestart' => $now - 8, 'timemodified' => 4]);

        // By default orders the discussions by last post.
        $summaries = array_values($vault->get_from_peerforum_id($peerforum->id, false, null, null,
                0, 0));
        ...
        $summaries = array_values($vault->get_from_peerforum_id($peerforum->id, true, null, null,
                0, 0));
        ...
        $summaries = array_values($vault->get_from_peerforum_id($peerforum->id, false, $user->id, null,
                0, 0));
        $this->assertCount(4, $summaries);
        $this->assertEquals($hiddendiscussion->id, $summaries[0]->get_discussion()->get_id());
        $this->assertEquals($discussion3->id, $summaries[1]->get_discussion()->get_id());
        ...
        $summaries = array_values($vault->get_from_peerforum_id($peerforum->id, true, null,
        ...
        $summaries = array_values($vault->get_from_peerforum_id($peerforum->id, true, $user->id,
                $vault::SORTORDER_REPLIES_DESC, 0, 0));
        $this->assertCount(4, $summaries);
        $this->assertEquals($discussion1->id, $summaries[0]->get_discussion()->get_id());
        $this->assertEquals($hiddendiscussion->id, $summaries[1]->get_discussion()->get_id());
    ...
    /**
     * Test get_from_peerforum_id_and_group_id.
     */
    public function test_get_from_peerforum_id_and_group_id() {
        $this->resetAfterTest();
        ...
        $this->assertEquals([], $vault->get_from_peerforum_id($peerforum->id, true, true,
                null, 0, 0, $user));
        ...
        [$hiddendiscussion, $post3] =
                $this->helper_post_to_peerforum($peerforum, $user, ['timestart' => $now + 10, 'timemodified' => 3]);
        [$groupdiscussion1, $post4] = $this->helper_post_to_peerforum(
                $peerforum,
                $user,
                ['timestart' => $now - 8, 'timemodified' => 4, 'groupid' => 1]
        );
        [$groupdiscussion2, $post5] = $this->helper_post_to_peerforum(
                $peerforum,
                $user,
                ['timestart' => $now - 7, 'timemodified' => 5, 'groupid' => 2]
        );
        [$hiddengroupdiscussion, $post6] = $this->helper_post_to_peerforum(
                $peerforum,
                $user,
                ['timestart' => $now + 11, 'timemodified' => 6, 'groupid' => 3]
        );

        $summaries = array_values($vault->get_from_peerforum_id_and_group_id($peerforum->id, [1, 2, 3], true,
                null, null, 0, 0));
        ...
        $summaries = array_values($vault->get_from_peerforum_id_and_group_id($peerforum->id, [1, 2, 3], false,
                $user->id, null, 0, 0));
        $this->assertCount(6, $summaries);
        $this->assertEquals($hiddengroupdiscussion->id, $summaries[0]->get_discussion()->get_id());
        $this->assertEquals($hiddendiscussion->id, $summaries[1]->get_discussion()->get_id());
        ...
        $summaries = array_values($vault->get_from_peerforum_id_and_group_id($peerforum->id, [], true,
                null, null, 0, 0));
        $this->assertCount(3, $summaries);
        $this->assertEquals($hiddendiscussion->id, $summaries[0]->get_discussion()->get_id());
        $this->assertEquals($discussion2->id, $summaries[1]->get_discussion()->get_id());
    ...
    /**
     * Test get_total_discussion_count_from_peerforum_id.
     */
    public function test_get_total_discussion_count_from_peerforum_id() {
        $this->resetAfterTest();
        ...
        $this->assertEquals(0, $vault->get_total_discussion_count_from_peerforum_id($peerforum->id, true,
                null));
        ...
        $this->assertEquals(2, $vault->get_total_discussion_count_from_peerforum_id($peerforum->id, false,
                null));
        $this->assertEquals(3, $vault->get_total_discussion_count_from_peerforum_id($peerforum->id, true,
                null));
        $this->assertEquals(3, $vault->get_total_discussion_count_from_peerforum_id($peerforum->id, false,
                $user->id));
    }

    /**
     * Test get_total_discussion_count_from_peerforum_id_and_group_id.
     */
    public function test_get_total_discussion_count_from_peerforum_id_and_group_id() {
        $this->resetAfterTest();
        ...
        $this->assertEquals([], $vault->get_from_peerforum_id($peerforum->id, true, null,
                null, 0, 0));
        ...
        $this->assertEquals(6, $vault->get_total_discussion_count_from_peerforum_id_and_group_id($peerforum->id, [1, 2, 3],
                true, null));
        $this->assertEquals(6, $vault->get_total_discussion_count_from_peerforum_id_and_group_id(
                $peerforum->id,
                [1, 2, 3],
                false,
                $user->id
        ));
        $this->assertEquals(4, $vault->get_total_discussion_count_from_peerforum_id_and_group_id($peerforum->id, [1, 2, 3],
                false, null));
        $this->assertEquals(3, $vault->get_total_discussion_count_from_peerforum_id_and_group_id($peerforum->id, [],
                true, null));
    }
    ...
    private function pin_discussion(stdClass $discussion) {
        ...
        $DB->update_record('peerforum_discussions',
                (object) array('id' => $discussion->id, 'pinned' => PEERFORUM_DISCUSSION_PINNED));
    }
    ...
    private function star_discussion(stdClass $discussion, bool $targetstate) {
        mod_peerforum_external::toggle_favourite_state($discussion->id, $targetstate);
    }
}
