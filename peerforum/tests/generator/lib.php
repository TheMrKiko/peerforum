...
class mod_peerforum_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->grade_peerforum)) {
            $record->grade_peerforum = 0;
        }
    ...
    public function create_discussion($record = null) {
        ...
        if (property_exists($record, 'tags')) {
            $cm = get_coursemodule_from_instance('peerforum', $record->peerforum);
            $tags = is_array($record->tags) ? $record->tags : preg_split('/,/', $record->tags);

            core_tag_tag::set_item_tags('mod_peerforum', 'peerforum_posts', $post->id,
    ...
    public function create_post($record = null) {
        ...
        if (property_exists($record, 'tags')) {
            $discussion = $DB->get_record('peerforum_discussions', ['id' => $record->discussion]);
            $cm = get_coursemodule_from_instance('peerforum', $discussion->peerforum);
            $tags = is_array($record->tags) ? $record->tags : preg_split('/,/', $record->tags);

            core_tag_tag::set_item_tags('mod_peerforum', 'peerforum_posts', $record->id,
                    context_module::instance($cm->id), $tags);
        }
    ...
    public function create_content($instance, $record = array()) {
        ...
        if (empty($record['discussion']) && empty($record['parent'])) {
            ...
            if (empty($record['parent'])) {
                $record['parent'] =
                        $DB->get_field('peerforum_discussions', 'firstpost', array('id' => $record['discussion']), MUST_EXIST);
            } else if (empty($record['discussion'])) {
                $record['discussion'] =
                        $DB->get_field('peerforum_posts', 'discussion', array('id' => $record['parent']), MUST_EXIST);
            }
