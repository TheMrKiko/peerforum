 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peerforum_exporters_post_testcase extends advanced_testcase {
    ...
    public function test_export_post($istimed = false, $addtime = 0) {
        ...
        if ($istimed) {
            $peerforumgenparams['timestart'] = $now + $addtime;
        }
        $discussion = $peerforumgenerator->create_discussion((object) $peerforumgenparams);

        $post = $peerforumgenerator->create_post((object) [
        ...
        \core_tag_tag::set_item_tags('mod_peerforum', 'peerforum_posts', $post->id, $context, ['foo', 'bar']);
        $tags = \core_tag_tag::get_item_tags('mod_peerforum', 'peerforum_posts', $post->id);
    ...
    public function test_export_deleted_post() {
        ...
        $coursemodule = get_coursemodule_from_instance('peerforum', $peerforum->id);
        $context = context_module::instance($coursemodule->id);
        $discussion = $peerforumgenerator->create_discussion((object) [
                'course' => $peerforum->course,
                'userid' => $user->id,
                'peerforum' => $peerforum->id
        ]);
