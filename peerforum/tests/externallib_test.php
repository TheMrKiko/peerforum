...
class mod_peerforum_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_mod_peerforum_get_peerforums_by_courses() {
        ...
        $peerforum1->istracked = true;
        $peerforum1->unreadpostscount = 0;
        $peerforum1->introfiles = [];
     ...
     */
    public function test_mod_peerforum_toggle_favourite_state() {
        global $USER, $CFG, $DB;
        ...
        $response = mod_peerforum_external::toggle_favourite_state($discussion1->id, 1);
        $response = external_api::clean_returnvalue(mod_peerforum_external::toggle_favourite_state_returns(), $response);
        $this->assertTrue($response['userstate']['favourited']);

        $response = mod_peerforum_external::toggle_favourite_state($discussion1->id, 0);
        $response = external_api::clean_returnvalue(mod_peerforum_external::toggle_favourite_state_returns(), $response);
     ...
     */
    public function test_mod_peerforum_set_pin_state() {
        $this->resetAfterTest(true);
        ...
        try {
            $response = mod_peerforum_external::set_pin_state($discussion1->id, 1);
        } catch (Exception $e) {
        ...
        self::setAdminUser();
        $response = mod_peerforum_external::set_pin_state($discussion1->id, 1);
        $response = external_api::clean_returnvalue(mod_peerforum_external::set_pin_state_returns(), $response);
        $this->assertTrue($response['pinned']);

        $response = mod_peerforum_external::set_pin_state($discussion1->id, 0);
        $response = external_api::clean_returnvalue(mod_peerforum_external::set_pin_state_returns(), $response);
    ...
    public function test_mod_peerforum_get_peerforum_discussion_posts() {
        ...
        // PeerForum with tracking enabled.
        $record = new stdClass();
        $record->course = $course1->id;
        $peerforum2 = self::getDataGenerator()->create_module('peerforum', $record);
        $peerforum2cm = get_coursemodule_from_id('peerforum', $peerforum2->cmid);
        $peerforum2context = context_module::instance($peerforum2->cmid);
        ...
        $filerecordinline = array(
                'contextid' => $peerforum1context->id,
        ...
        $expectedposts['posts'][] = array(
                ...
                'tags' => \core_tag\external\util::get_item_tags('mod_peerforum', 'peerforum_posts', $discussion1reply2->id),
        );
        // Cast to expected.
        ...
        $expectedposts['posts'][] = array(
                ...
                'messageinlinefiles' => array(
                        array(
                                ...
                                'filepath' => '/',
                                'filesize' => '27',
                                'fileurl' => moodle_url::make_webservice_pluginfile_url($peerforum1context->id, 'mod_peerforum',
                                        'post',
        ...
        // Check we receive the unread count correctly on tracked peerforum.
        peerforum_tp_count_peerforum_unread_posts($peerforum2cm, $course1, true);    // Reset static cache.
        $result = mod_peerforum_external::get_peerforums_by_courses(array($course1->id));
        $result = external_api::clean_returnvalue(mod_peerforum_external::get_peerforums_by_courses_returns(), $result);
        foreach ($result as $f) {
            if ($f['id'] == $peerforum2->id) {
        ...
        // Test discussion tracking on not tracked peerforum.
        $result = mod_peerforum_external::view_peerforum_discussion($discussion1->id);
        $result = external_api::clean_returnvalue(mod_peerforum_external::view_peerforum_discussion_returns(), $result);
        ...
        // Test discussion tracking on tracked peerforum.
        $result = mod_peerforum_external::view_peerforum_discussion($discussion3->id);
        $result = external_api::clean_returnvalue(mod_peerforum_external::view_peerforum_discussion_returns(), $result);
        $this->assertTrue($result['status']);
        $this->assertEmpty($result['warnings']);

        // Test posts have been marked as read.
        $posts = mod_peerforum_external::get_peerforum_discussion_posts($discussion3->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussion_posts_returns(), $posts);
        foreach ($posts['posts'] as $post) {
     ...
     * Test get peerforum posts
     *
     * Tests is similar to the get_peerforum_discussion_posts only utilizing the new return structure and entities
     */
    public function test_mod_peerforum_get_discussion_posts() {
        global $CFG;
        ...
        $legacyfactory = mod_peerforum\local\container::get_legacy_data_mapper_factory();
        ...
        $peerforumgenerator = self::getDataGenerator()->get_plugin_generator('mod_peerforum');
        ...
        $record->course = $course1->id;
        $record->trackingtype = PEERFORUM_TRACKING_OFF;
        // Display word count. Otherwise, word and char counts will be set to null by the peerforum post exporter.
        ...
        $record->userid = $user1->id;
        $record->peerforum = $peerforum1->id;
        $discussion1 = $peerforumgenerator->create_discussion($record);

        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user2->id;
        $record->peerforum = $peerforum1->id;
        $discussion2 = $peerforumgenerator->create_discussion($record);

        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user2->id;
        $record->peerforum = $peerforum2->id;
        $discussion3 = $peerforumgenerator->create_discussion($record);
        ...
        $record->parent = $discussion1->firstpost;
        $record->userid = $user2->id;
        $discussion1reply1 = $peerforumgenerator->create_post($record);
        $filename = 'shouldbeanimage.jpg';
        // Add a fake inline image to the post.
        $filerecordinline = array(
                'contextid' => $peerforum1context->id,
        ...
        $record->parent = $discussion1reply1->id;
        $record->userid = $user3->id;
        $discussion1reply2 = $peerforumgenerator->create_post($record);
        ...
        $exporteduser3 = [
                'id' => (int) $user3->id,
                'fullname' => get_string('deleteduser', 'mod_peerforum'),
                'groups' => [],
                'isdeleted' => true,
                'urls' => [
                        'profile' => $urlfactory->get_author_profile_url($user3entity, $course1->id)->out(false),
        ...
        $expectedposts['posts'][] = array(
                ...
                'subject' => $discussion1reply2->subject,
                'replysubject' => get_string('re', 'mod_peerforum') . " {$discussion1reply2->subject}",
                'message' => $message,
                'messageformat' => 1,   // This value is usually changed by external_format_text() function.
                'unread' => null,
                'isdeleted' => false,
                ...
                'html' => [
                        'rating' => null,
                        'taglist' => null,
                        'authorsubheading' => $peerforumgenerator->get_author_subheading_html((object) $exporteduser3,
                ...
                'urls' => [
                        'view' => $urlfactory->get_view_post_url_from_post_id($discussion1reply2->discussion,
                                $discussion1reply2->id),
                        ...
                        'delete' => null,
                        'split' => null,
                        'reply' => (new moodle_url('/mod/peerforum/post.php#mformpeerforum', [
        ...
        $expectedposts['posts'][] = array(
                ...
                'subject' => $discussion1reply1->subject,
                'replysubject' => get_string('re', 'mod_peerforum') . " {$discussion1reply1->subject}",
                'message' => $message,
                'messageformat' => 1,   // This value is usually changed by external_format_text() function.
                'unread' => null,
                'isdeleted' => false,
                ...
                'html' => [
                        'rating' => null,
                        'taglist' => null,
                        'authorsubheading' => $peerforumgenerator->get_author_subheading_html((object) $exporteduser2,
                ...
                'urls' => [
                        'view' => $urlfactory->get_view_post_url_from_post_id($discussion1reply1->discussion,
        ...
        // Test a discussion with two additional posts (total 3 posts).
        $posts = mod_peerforum_external::get_discussion_posts($discussion1->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_peerforum_external::get_discussion_posts_returns(), $posts);
        $this->assertEquals(3, count($posts['posts']));
        ...
        // Test discussion without additional posts. There should be only one post (the one created by the discussion).
        $posts = mod_peerforum_external::get_discussion_posts($discussion2->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_peerforum_external::get_discussion_posts_returns(), $posts);
        $this->assertEquals(1, count($posts['posts']));
        ...
        // Test posts have been marked as read.
        $posts = mod_peerforum_external::get_discussion_posts($discussion3->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_peerforum_external::get_discussion_posts_returns(), $posts);
        foreach ($posts['posts'] as $post) {
     ...
     * Test get peerforum posts
     */
    public function test_mod_peerforum_get_peerforum_discussion_posts_deleted() {
        global $CFG, $PAGE;
        ...
        // Create test data.
        $peerforum1 = self::getDataGenerator()->create_module('peerforum', (object) [
                'course' => $course1->id,
        ]);
        $peerforum1context = context_module::instance($peerforum1->cmid);

        // Add discussions to the peerforum.
        ...
        $posts = mod_peerforum_external::get_peerforum_discussion_posts($discussion->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussion_posts_returns(), $posts);
        $deletedsubject = get_string('privacy:request:delete:post:subject', 'mod_peerforum');
        $deletedmessage = get_string('privacy:request:delete:post:message', 'mod_peerforum');
    ...
    public function test_mod_peerforum_get_peerforum_discussions_paginated() {
        ...
        $expecteddiscussions = array(
                ...
                'numreplies' => 3,
                'numunread' => 0,
                'pinned' => (bool) PEERFORUM_DISCUSSION_UNPINNED,
                'locked' => false,
                'canreply' => false,
                'canlock' => false
        );
        ...
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_paginated_returns(),
                $discussions);
    ...
    public function test_mod_peerforum_get_peerforum_discussions() {
        ...
        // Call the external function passing peerforum id.
        $discussions = mod_peerforum_external::get_peerforum_discussions($peerforum1->id);
        ...
        $t = mod_peerforum_external::toggle_favourite_state($discussion1->id, 1);
        ...
        $this->unassignUserCapability('mod/peerforum:viewdiscussion', $context->id, $newrole);
        try {
            mod_peerforum_external::get_peerforum_discussions($peerforum1->id);
            $this->fail('Exception expected due to missing capability.');
        } catch (moodle_exception $e) {
            $this->assertEquals('noviewdiscussionspermission', $e->errorcode);
        }
    ...
    /**
     * Test the sorting in get peerforum discussions
     */
    public function test_mod_peerforum_get_peerforum_discussions_sorting() {
        global $CFG, $DB, $PAGE;
        ...
        $record2->peerforum = $peerforum1->id;
        $discussion2 = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record2);
        sleep(1);
        ...
        $discussion2reply1 = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_post($record2);
        sleep(1);
        ...
        $record3->peerforum = $peerforum1->id;
        $discussion3 = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record3);
        sleep(1);
        ...
        $discussion3reply1 = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_post($record3);
        sleep(1);

        $record3->parent = $discussion3reply1->id;
        $record3->userid = $user3->id;
        $discussion3reply2 = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_post($record3);
        ...
        // Call the external function passing peerforum id and sort order parameter.
        $discussions =
                mod_peerforum_external::get_peerforum_discussions($peerforum1->id, $discussionlistvault::SORTORDER_LASTPOST_ASC);
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_returns(), $discussions);
        ...
        // Call the external function passing peerforum id and sort order parameter.
        $discussions =
                mod_peerforum_external::get_peerforum_discussions($peerforum1->id, $discussionlistvault::SORTORDER_CREATED_DESC);
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_returns(), $discussions);
        ...
        // Call the external function passing peerforum id and sort order parameter.
        $discussions =
                mod_peerforum_external::get_peerforum_discussions($peerforum1->id, $discussionlistvault::SORTORDER_CREATED_ASC);
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_returns(), $discussions);
        ...
        // Call the external function passing peerforum id and sort order parameter.
        $discussions =
                mod_peerforum_external::get_peerforum_discussions($peerforum1->id, $discussionlistvault::SORTORDER_REPLIES_DESC);
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_returns(), $discussions);
        ...
        // Call the external function passing peerforum id and sort order parameter.
        $discussions =
                mod_peerforum_external::get_peerforum_discussions($peerforum1->id, $discussionlistvault::SORTORDER_REPLIES_ASC);
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_returns(), $discussions);
        ...
        $DB->update_record('peerforum_discussions',
                (object) array('id' => $discussion2->id, 'pinned' => PEERFORUM_DISCUSSION_PINNED));
    ...
    public function test_add_discussion_post() {
        ...
        $createdpost = mod_peerforum_external::add_discussion_post($discussion->firstpost, 'some subject', 'some text here...');
        $createdpost = external_api::clean_returnvalue(mod_peerforum_external::add_discussion_post_returns(), $createdpost);
        ...
        foreach ($formats as $format) {
            $createdpost = mod_peerforum_external::add_discussion_post($discussion->firstpost,
                    'with some format', 'some formatted here...', $options, $format);
            $createdpost = external_api::clean_returnvalue(mod_peerforum_external::add_discussion_post_returns(), $createdpost);
            $dbformat = $DB->get_field('peerforum_posts', 'messageformat', ['id' => $createdpost['postid']]);
        ...
        $createdpost = external_api::clean_returnvalue(mod_peerforum_external::add_discussion_post_returns(), $createdpost);
        $dbpost = $DB->get_record('peerforum_posts', ['id' => $createdpost['postid']]);
        ...
        $dummytext = 'Here is an inline image: <img src="' . $CFG->wwwroot
                ...
                . '" alt="inlineimage">.';
        $createdpost = mod_peerforum_external::add_discussion_post($discussion->firstpost, 'new post inline attachment',
                $dummytext, $options);
        $createdpost = external_api::clean_returnvalue(mod_peerforum_external::add_discussion_post_returns(), $createdpost);
    ...
    public function test_add_discussion_post_subscribe_discussion() {
        ...
        // Add a discussion post in a peerforum discussion where the user is not subscribed (auto-subscribe preference enabled).
        mod_peerforum_external::add_discussion_post($discussion1->firstpost, 'some subject', 'some text here...');

        $posts = mod_peerforum_external::get_peerforum_discussion_posts($discussion1->id);
        $posts = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussion_posts_returns(), $posts);
        // We receive the discussion and the post.
        $this->assertEquals(2, count($posts['posts']));
        // The user should be subscribed to the discussion after adding a discussion post.
        $this->assertTrue(\mod_peerforum\subscriptions::is_subscribed($user->id, $peerforum, $discussion1->id, $cm));

        // Disable auto subscribe discussion.
        $USER->autosubscribe = false;
        $this->assertTrue(\mod_peerforum\subscriptions::is_subscribed($user->id, $peerforum, $discussion1->id, $cm));
        // Add a discussion post in a peerforum discussion where the user is subscribed (auto-subscribe preference disabled).
        mod_peerforum_external::add_discussion_post($discussion1->firstpost, 'some subject 1', 'some text here 1...');
        ...
        $this->assertFalse(\mod_peerforum\subscriptions::is_subscribed($user->id, $peerforum, $discussion2->id, $cm));
        // Add a discussion post in a peerforum discussion where the user is not subscribed (auto-subscribe preference disabled).
        mod_peerforum_external::add_discussion_post($discussion2->firstpost, 'some subject 2', 'some text here 2...');

        $posts = mod_peerforum_external::get_peerforum_discussion_posts($discussion2->id);
        $posts = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussion_posts_returns(), $posts);
        // We receive the discussion and the post.
        $this->assertEquals(2, count($posts['posts']));
        ...
        $this->assertTrue(\mod_peerforum\subscriptions::is_subscribed($user->id, $peerforum, $discussion2->id, $cm));
    }
    ...
    public function test_add_discussion() {
        ...
        self::setAdminUser();
        $createddiscussion = mod_peerforum_external::add_discussion($peerforum->id, 'the subject', 'some text here...');
        $createddiscussion = external_api::clean_returnvalue(mod_peerforum_external::add_discussion_returns(), $createddiscussion);
        ...
        $discussion2pinned = mod_peerforum_external::add_discussion($peerforum->id, 'the pinned subject', 'some 2 text here...', -1,
                array('options' => array('name' => 'discussionpinned',
                        'value' => true)));
        $discussion3 = mod_peerforum_external::add_discussion($peerforum->id, 'the non pinnedsubject', 'some 3 text here...');
        $discussions = mod_peerforum_external::get_peerforum_discussions_paginated($peerforum->id);
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_paginated_returns(),
                $discussions);
        ...
        $options = array(array('name' => 'inlineattachmentsid', 'value' => $draftidinlineattach),
                array('name' => 'attachmentsid', 'value' => $draftidattach));
        $createddiscussion = mod_peerforum_external::add_discussion($peerforum->id, 'the inline attachment subject',
                $dummytext, -1, $options);
        $createddiscussion = external_api::clean_returnvalue(mod_peerforum_external::add_discussion_returns(), $createddiscussion);
    ...
    public function test_set_lock_state() {
        ...
        try {
            $result = mod_peerforum_external::set_lock_state($peerforum->id, $discussion->id, 0);
            $this->fail('Exception expected due to missing capability.');
        } catch (moodle_exception $e) {
        ...
        self::setAdminUser();
        $result = mod_peerforum_external::set_lock_state($peerforum->id, $discussion->id, 0);
        $result = external_api::clean_returnvalue(mod_peerforum_external::set_lock_state_returns(), $result);
        $this->assertTrue($result['locked']);
        $this->assertNotEquals(0, $result['times']['locked']);

        // Unset the lock.
        $result = mod_peerforum_external::set_lock_state($peerforum->id, $discussion->id, time());
        $result = external_api::clean_returnvalue(mod_peerforum_external::set_lock_state_returns(), $result);
    ...
    public function test_can_add_discussion() {
        ...
        // Disable attachments.
        $DB->set_field('peerforum', 'maxattachments', 0, array('id' => $peerforum->id));
        $result = mod_peerforum_external::can_add_discussion($peerforum->id);
        $result = external_api::clean_returnvalue(mod_peerforum_external::can_add_discussion_returns(), $result);
        $this->assertFalse($result['status']);
        $this->assertFalse($result['canpindiscussions']);
    ...
    /*
     * A basic test to make sure users cannot post to peerforum after the cutoff date.
     */
    public function test_can_add_discussion_after_cutoff() {
        ...
        // Create a peerforum with cutoff date set to a past date.
        $peerforum = self::getDataGenerator()->create_module('peerforum', ['course' => $course->id, 'cutoffdate' => time() - 1]);

        // User with no mod/peerforum:canoverridecutoff capability.
        self::setUser($user);
        $this->getDataGenerator()->enrol_user($user->id, $course->id);
    ...
    /**
     * Test get peerforum posts discussions including rating information.
     */
    public function test_mod_peerforum_get_peerforum_discussion_rating_information() {
        global $DB, $CFG;
        ...
        // Add discussion to the peerforum.
        $record = new stdClass();
        $record->course = $course->id;
        $record->userid = $user1->id;
        $record->peerforum = $peerforum->id;
        ...
        $rating1->component = 'mod_peerforum';
        ...
        $rating2->component = 'mod_peerforum';
    ...
    /**
     * Test mod_peerforum_get_peerforum_access_information.
     */
    public function test_mod_peerforum_get_peerforum_access_information() {
        global $DB;
        ...
        $result = mod_peerforum_external::get_peerforum_access_information($peerforum->id);
        $result = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_access_information_returns(), $result);
        ...
        unassign_capability('mod/peerforum:deleteownpost', $studentrole->id);
        unassign_capability('mod/peerforum:allowforcesubscribe', $studentrole->id);
    ...
    public function test_add_discussion_post_private() {
        ...
        // Add a new discussion to the peerforum.
        ...
        $privatereply = $DB->get_record('peerforum_posts', array('id' => $post['postid']));
        ...
        $DB->update_record('peerforum_posts', $privatereply);
        ...
        $post = mod_peerforum_external::add_discussion_post($privatereply->id, 'some subject', 'some text here...', [
                'options' => [
                        'name' => 'private',
                        'value' => false,
                ],
    ...
    public function test_trusted_text_enabled() {
        ...
        $discussion1 = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($discussionrecord);

        self::setAdminUser();
        $discussionrecord->userid = $USER->id;
        $discussionrecord->messagetrust = trusttext_trusted($context);
        $discussion2 = self::getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($discussionrecord);
    ...
    public function test_trusted_text_disabled() {
        ...
        $discussions = mod_peerforum_external::get_peerforum_discussions($peerforum->id);
        $discussions = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussions_returns(), $discussions);
    ...
    public function test_delete_post_discussion() {
        ...
        $this->setUser($user);
        $result = mod_peerforum_external::delete_post($discussion->firstpost);
        $result = external_api::clean_returnvalue(mod_peerforum_external::delete_post_returns(), $result);
        $this->assertTrue($result['status']);
        $this->assertEquals(0, $DB->count_records('peerforum_posts', array('id' => $discussion->firstpost)));
        $this->assertEquals(0, $DB->count_records('peerforum_discussions', array('id' => $discussion->id)));
    }
    ...
    public function test_delete_post_post() {
        ...
        $this->setUser($user);
        $result = mod_peerforum_external::delete_post($post->id);
        $result = external_api::clean_returnvalue(mod_peerforum_external::delete_post_returns(), $result);
        $this->assertTrue($result['status']);
        $this->assertEquals(1, $DB->count_records('peerforum_posts', array('discussion' => $discussion->id)));
        $this->assertEquals(1, $DB->count_records('peerforum_discussions', array('id' => $discussion->id)));
    }
    ...
    public function test_delete_post_other_user_post() {
        ...
        $this->expectExceptionMessage(get_string('cannotdeletepost', 'peerforum'));
        mod_peerforum_external::delete_post($post->id);
    }

    /*
     * Test get peerforum posts by user id.
     */
    public function test_mod_peerforum_get_discussion_posts_by_userid() {
        global $DB;
        $this->resetAfterTest(true);
        ...
        $record->userid = $user2->id;
        $discussion2reply1 = $peerforumgenerator->create_post($record);
        $filename = 'shouldbeanimage.jpg';
        // Add a fake inline image to the post.
        $filerecordinline = array(
                'contextid' => $peerforum1context->id,
                ...
                'posts' => [
                        'userposts' => [
                                [
                                        ...
                                        'html' => [
                                                'rating' => null,
                                                'taglist' => null,
                                                'authorsubheading' => $peerforumgenerator->get_author_subheading_html(
                                        ...
                                        'urls' => [
                                                ...
                                                'edit' => (new moodle_url('/mod/peerforum/post.php', [
                                                        'edit' => $discussion1reply1->id
                                                ]))->out(false),
                                                'delete' => (new moodle_url('/mod/peerforum/post.php', [
                                                        'delete' => $discussion1reply1->id
                                                ]))->out(false),
                                                'split' => (new moodle_url('/mod/peerforum/post.php', [
                        ...
                        'parentposts' => [
                                [
                                        ...
                                        'replysubject' => get_string('re', 'mod_peerforum') .
                                                " {$discussion1firstpostobject->subject}",
                                        'message' => file_rewrite_pluginfile_urls($discussion1firstpostobject->message,
                                                'pluginfile.php',
                                                $peerforum1context->id, 'mod_peerforum', 'post', $discussion1firstpostobject->id),
                                        'messageformat' => 1,   // This value is usually changed by external_format_text() function.
                                        'unread' => null,
                                        'isdeleted' => false,
                                        'isprivatereply' => false,
                                        ...
                                        'urls' => [
                                                'view' => $urlfactory->get_view_post_url_from_post_id(
                                                        $discussion1firstpostobject->discussion, $discussion1firstpostobject->id)
                                                        ->out(false),
                ...
                'posts' => [
                        'userposts' => [
                                [
                                        ...
                                        'replysubject' => get_string('re', 'mod_peerforum') . " {$discussion2reply1->subject}",
                                        'message' => file_rewrite_pluginfile_urls($discussion2reply1->message, 'pluginfile.php',
                                                $peerforum1context->id, 'mod_peerforum', 'post', $discussion2reply1->id),
                                        'messageformat' => 1,   // This value is usually changed by external_format_text() function.
                                        'unread' => null,
                                        'isdeleted' => false,
                                        'isprivatereply' => false,
                        ...
                        'parentposts' => [
                                [
                                        ...
                                        'replysubject' => get_string('re', 'mod_peerforum') .
                                                " {$discussion2firstpostobject->subject}",
                                        'message' => file_rewrite_pluginfile_urls($discussion2firstpostobject->message,
                                                'pluginfile.php',
                                                $peerforum1context->id, 'mod_peerforum', 'post', $discussion2firstpostobject->id),
                                        'messageformat' => 1,   // This value is usually changed by external_format_text() function.
                                        'unread' => null,
                                        'isdeleted' => false,
                                        'isprivatereply' => false,
                                        ...
                                        'urls' => [
                                                'view' => $urlfactory->get_view_post_url_from_post_id(
                                                        $discussion2firstpostobject->discussion, $discussion2firstpostobject->id)
                                                        ->out(false),
                                                'viewisolated' => $isolatedurlparent->out(false),
                                                'viewparent' => null,
                                                'edit' => (new moodle_url('/mod/peerforum/post.php', [
        ...
        $discussions = mod_peerforum_external::get_discussion_posts_by_userid($user2->id, $peerforum1->cmid, 'modified', 'DESC');
        $discussions =
                external_api::clean_returnvalue(mod_peerforum_external::get_discussion_posts_by_userid_returns(), $discussions);
    ...
    public function test_get_discussion_post_discussion() {
        ...
        $discussion = $this->getDataGenerator()->get_plugin_generator('mod_peerforum')->create_discussion($record);
        $this->setUser($user);
        $result = mod_peerforum_external::get_discussion_post($discussion->firstpost);
        $result = external_api::clean_returnvalue(mod_peerforum_external::get_discussion_post_returns(), $result);
    ...
    public function test_get_discussion_post_post() {
        ...
        $post = $this->getDataGenerator()->get_plugin_generator('mod_peerforum')->create_post($record);
        $this->setUser($user);
        $result = mod_peerforum_external::get_discussion_post($post->id);
        $result = external_api::clean_returnvalue(mod_peerforum_external::get_discussion_post_returns(), $result);
    ...
    public function test_prepare_draft_area_for_post() {
        ...
        $result = mod_peerforum_external::prepare_draft_area_for_post($post->id, 'attachment');
        $result = external_api::clean_returnvalue(mod_peerforum_external::prepare_draft_area_for_post_returns(), $result);
        ...
        $result = mod_peerforum_external::prepare_draft_area_for_post($post->id, 'attachment', $result['draftitemid']);
        $result = external_api::clean_returnvalue(mod_peerforum_external::prepare_draft_area_for_post_returns(), $result);
        $this->assertCount(2, $result['files']);
        ...
        $result =
                mod_peerforum_external::prepare_draft_area_for_post($post->id, 'attachment', $result['draftitemid'], $filestokeep);
        $result = external_api::clean_returnvalue(mod_peerforum_external::prepare_draft_area_for_post_returns(), $result);
        ...
        $result = mod_peerforum_external::prepare_draft_area_for_post($post->id, 'post');
        $result = external_api::clean_returnvalue(mod_peerforum_external::prepare_draft_area_for_post_returns(), $result);
        $this->assertCount(1, $result['files']);
    ...
    public function test_update_discussion_post_discussion() {
        ...
        $result = mod_peerforum_external::update_discussion_post($discussion->firstpost, $subject, $message, $messageformat,
                $options);
        $result = external_api::clean_returnvalue(mod_peerforum_external::update_discussion_post_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals(PEERFORUM_DISCUSSION_PINNED, $discussion->pinned);  // Check discussion pinned.
    }
    ...
    public function test_update_discussion_post_post() {
        ...
        $result = mod_peerforum_external::add_discussion_post($discussion->firstpost, 'some subject', 'some text here...');
        $newpost = $result['post'];
        $this->assertTrue(\mod_peerforum\subscriptions::is_subscribed($user->id, $peerforum, $discussion->id, $cm));
        ...
        $result = mod_peerforum_external::update_discussion_post($newpost->id, '', $message, $messageformat, $options);
        $result = external_api::clean_returnvalue(mod_peerforum_external::update_discussion_post_returns(), $result);
        $this->assertTrue($result['status']);
        // Check subscription status.
        $this->assertFalse(\mod_peerforum\subscriptions::is_subscribed($user->id, $peerforum, $discussion->id, $cm));

        // Get the post from WS.
        $result = mod_peerforum_external::get_peerforum_discussion_posts($discussion->id);
        $result = external_api::clean_returnvalue(mod_peerforum_external::get_peerforum_discussion_posts_returns(), $result);
        $found = false;
    ...
    public function test_update_discussion_post_other_user_post() {
        ...
        $newpost = $this->getDataGenerator()->get_plugin_generator('mod_peerforum')->create_post($record);
        ...
        $this->expectExceptionMessage(get_string('cannotupdatepost', 'peerforum'));
        mod_peerforum_external::update_discussion_post($newpost->id, $subject, $message, $messageformat);
    }
}
