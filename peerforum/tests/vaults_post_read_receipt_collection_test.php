 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peerforum_vaults_post_read_receipt_collection_testcase extends advanced_testcase {
    ...
    public function test_get_from_user_id_and_post_ids() {
        ...
        [$discussion2, $post2] = $this->helper_post_to_peerforum($peerforum, $user);
        $post3 = $this->helper_reply_to_post($post2, $user);

        $DB->insert_record('peerforum_read', [
                'userid' => $user->id,
