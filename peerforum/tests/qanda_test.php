...
/**
 * The peerforum module mail generation tests for groups.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peerforum_qanda_testcase extends advanced_testcase {
    ...
    /**
     * Test that a user who has not posted in a q&a peerforum does not receive
     * notificatinos.
     */
    public function test_user_has_not_posted() {
        ...
        list($discussion, $post) = $this->helper_post_to_peerforum($peerforum, $editingteacher);
        $reply = $this->helper_reply_to_post($post, $author);
        $otherreply = $this->helper_reply_to_post($post, $recipient);
        $DB->execute("UPDATE {peerforum_posts} SET modified = modified - 1");
        $DB->execute("UPDATE {peerforum_posts} SET created = created - 1");
        $DB->execute("UPDATE {peerforum_discussions} SET timemodified = timemodified - 1");
