 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peerforum_builders_exported_posts_testcase extends advanced_testcase {
    // Make use of the test generator trait.
    use mod_peerforum_tests_generator_trait;

    /** @var \mod_peerforum\local\builders\exported_posts */
    private $builder;
     ...
     *
     * @param stdClass[] $peerforums List of peerforums
     * @param stdClass[] $discussions List of discussions
     * @param stdClass[] $posts List of posts
     * @return array
     */
    private function convert_to_entities(array $peerforums, array $discussions, array $posts) {
        global $DB;
        $entityfactory = \mod_peerforum\local\container::get_entity_factory();

        return [
            // PeerForums.
                array_map(function($peerforum) use ($entityfactory, $DB) {
                    $course = $DB->get_record('course', ['id' => $peerforum->course]);
                    $coursemodule = get_coursemodule_from_instance('peerforum', $peerforum->id);
                    $context = context_module::instance($coursemodule->id);
                    return $entityfactory->get_peerforum_from_stdclass($peerforum, $context, $coursemodule, $course);
                }, $peerforums),
            // Discussions.
    ...
    /**
     * Test the build function throws exception if not given all of the peerforums for
     * the list of posts.
     */
    public function test_build_throws_exception_on_missing_peerforums() {
        $this->resetAfterTest();
        ...
        [$discussion1, $post1] = $this->helper_post_to_peerforum($peerforum1, $user);
        [$discussion2, $post2] = $this->helper_post_to_peerforum($peerforum2, $user);

        [$peerforums, $discussions, $posts] = $this->convert_to_entities(
                [$peerforum1, $peerforum2],
                [$discussion1, $discussion2],
                [$post1, $post2]
        );

        $this->expectException('moodle_exception');
        $this->builder->build($user, [$peerforums[0]], $discussions, $posts);
    }
    ...
    public function test_build_throws_exception_on_missing_discussions() {
        ...
        $this->expectException('moodle_exception');
        $this->builder->build($user, $peerforums, [$discussions[0]], $posts);
    }
    ...
    public function test_build_returns_posts_in_order() {
        ...
        $peerforum1 = $datagenerator->create_module('peerforum', ['course' => $course->id]);
        $peerforum2 = $datagenerator->create_module('peerforum', ['course' => $course->id]);
        [$discussion1, $post1] = $this->helper_post_to_peerforum($peerforum1, $user1);
        [$discussion2, $post2] = $this->helper_post_to_peerforum($peerforum1, $user2);
        $post3 = $this->helper_reply_to_post($post1, $user1);
        $post4 = $this->helper_reply_to_post($post1, $user2);
        [$discussion3, $post5] = $this->helper_post_to_peerforum($peerforum2, $user1);
        [$discussion4, $post6] = $this->helper_post_to_peerforum($peerforum2, $user2);
        ...
        $exportedposts = $this->builder->build($user1, $peerforums, $discussions, $posts);
    ...
    public function test_build_loads_attachments() {
        ...
        // Add an attachment to a post in peerforum 1.
        $attachment1 = $filestorage->create_file_from_string(
                [
                        'contextid' => $peerforums[0]->get_context()->id,
        ...
        // Add an attachment to a post in peerforum 2.
        $attachment2 = $filestorage->create_file_from_string(
                [
                        'contextid' => $peerforums[1]->get_context()->id,
                        'component' => 'mod_peerforum',
                        'filearea' => 'attachment',
                        'itemid' => $post7->id,
                        'filepath' => '/',
    ...
    public function test_build_loads_author_groups() {
        ...
        $peerforum1 = $datagenerator->create_module('peerforum', ['course' => $course1->id]);
        $peerforum2 = $datagenerator->create_module('peerforum', ['course' => $course1->id]);
        [$discussion1, $post1] = $this->helper_post_to_peerforum($peerforum1, $user1);
        [$discussion2, $post2] = $this->helper_post_to_peerforum($peerforum1, $user2);
        $post3 = $this->helper_reply_to_post($post1, $user1);
        $post4 = $this->helper_reply_to_post($post1, $user2);
    ...
    public function test_build_loads_tags() {
        ...
        \core_tag_tag::set_item_tags('mod_peerforum', 'peerforum_posts', $post1->id, $peerforums[0]->get_context(), ['foo', 'bar']);
        \core_tag_tag::set_item_tags('mod_peerforum', 'peerforum_posts', $post4->id, $peerforums[0]->get_context(), ['foo', 'baz']);
        \core_tag_tag::set_item_tags('mod_peerforum', 'peerforum_posts', $post7->id, $peerforums[1]->get_context(), ['bip']);
    ...
    public function test_build_loads_read_receipts() {
        ...
        $peerforum1 = $datagenerator->create_module('peerforum',
                ['course' => $course1->id, 'trackingtype' => PEERFORUM_TRACKING_OPTIONAL]);
        $peerforum2 =
                $datagenerator->create_module('peerforum', ['course' => $course1->id, 'trackingtype' => PEERFORUM_TRACKING_OFF]);
        [$discussion1, $post1] = $this->helper_post_to_peerforum($peerforum1, $user1);
        [$discussion2, $post2] = $this->helper_post_to_peerforum($peerforum1, $user2);
        $post3 = $this->helper_reply_to_post($post1, $user1);
        $post4 = $this->helper_reply_to_post($post1, $user2);
        ...
        peerforum_tp_add_read_record($user1->id, $post1->id);
        peerforum_tp_add_read_record($user1->id, $post4->id);
        peerforum_tp_add_read_record($user1->id, $post7->id);
        peerforum_tp_add_read_record($user2->id, $post1->id);
        peerforum_tp_add_read_record($user2->id, $post4->id);
        peerforum_tp_add_read_record($user2->id, $post7->id);

        // User 1 has tracking enabled.
        $exportedposts = $this->builder->build($user1, $peerforums, $discussions, $posts);

        $expected = [
            // Tracking set for peerforum 1 for user 1.
                $post1->id => false,
                $post2->id => true,
                $post3->id => true,
                $post4->id => false,
            // Tracking is off for peerforum 2 so everything should be null.
                $post5->id => null,
                $post6->id => null,
                $post7->id => null,
                $post8->id => null
        ...
        $exportedposts = $this->builder->build($user2, $peerforums, $discussions, $posts);
