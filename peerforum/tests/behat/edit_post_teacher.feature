@mod @mod_peerforum
Feature: Teachers can edit or delete any peerforum post
  In order to refine the peerforum contents
  As a teacher
  I need to edit or delete any user's peerforum posts
  ...
  Background:
    ...
    And I reply "Teacher post subject" post from "Test peerforum name" peerforum with:
  ...
  Scenario: A teacher can delete another user's posts
    ...
    And I click on "Delete" "link" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][contains(., 'Student post subject')]" "xpath_element"
  ...
  Scenario: A teacher can edit another user's posts
    ...
    And I follow "Test peerforum name"
    And I follow "Teacher post subject"
    And I click on "Edit" "link" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][contains(., 'Student post subject')]" "xpath_element"
  ...
  Scenario: A student can't edit or delete another user's posts
    ...
    Then I should not see "Edit" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][contains(., 'Teacher post subject')]" "xpath_element"
    And I should not see "Delete" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][contains(., 'Teacher post subject')]" "xpath_element"
