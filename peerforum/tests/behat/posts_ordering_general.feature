...
Feature: New discussions and discussions with recently added replies are displayed first
  ...
  Scenario: Replying to a peerforum post or editing it puts the discussion to the front
    ...
    And I reply "PeerForum post 1" post from "Course general peerforum" peerforum with:
      | Message | Reply to the first post |
    And I am on "Course 1" course homepage
    And I follow "Course general peerforum"
    #
    # Make sure the order of the peerforum posts is as expected, with most recent new participation first (ie excluding edits).
    #
    Then I should see "PeerForum post 1" in the "//tr[contains(concat(' ', normalize-space(@class), ' '), ' discussion ')][position()=1]" "xpath_element"
    And I should see "PeerForum post 3" in the "//tr[contains(concat(' ', normalize-space(@class), ' '), ' discussion ')][position()=2]" "xpath_element"
    And I should see "Edited peerforum post 2" in the "//tr[contains(concat(' ', normalize-space(@class), ' '), ' discussion ')][position()=3]" "xpath_element"
    #
    # Make sure the next/prev navigation uses the same order of the posts.
    #
    And I follow "PeerForum post 3"
    And "//a[@aria-label='Next discussion: PeerForum post 1']" "xpath_element" should exist
    And "//a[@aria-label='Previous discussion: Edited peerforum post 2']" "xpath_element" should exist
