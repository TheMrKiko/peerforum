...
Feature: PeerForum discussions can be split
  ...
  Background:
    ...
    And I reply "Photosynthesis discussion" post from "Study discussions" peerforum with:
    ...
    And I click on "Reply" "link" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][contains(., 'Can anyone tell me which number is the mass number in the periodic table?')]" "xpath_element"
