...
Feature: Blog posts are always displayed in reverse chronological order
  ...
  Scenario: Replying to a blog post or editing it does not affect its display order
    ...
    #
    Then I should see "This is the third post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][position()=1]" "xpath_element"
    And I should see "This is the second post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][position()=2]" "xpath_element"
    And I should see "This is the first post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' peerforumpost ')][position()=3]" "xpath_element"
    #
