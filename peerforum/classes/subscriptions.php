...
class subscriptions {
    ...
    public static function is_subscribable($peerforum) {
        return (isloggedin() && !isguestuser() &&
                !\mod_peerforum\subscriptions::is_forcesubscribed($peerforum) &&
        ...
        if (self::is_forcesubscribed($peerforum)) {
            ...
            if ($includediscussionsubscriptions) {
                ...
                $sql = "SELECT $fields
                        ...
                        WHERE
                          s.peerforum = :peerforumid AND u.auth <> 'nologin' AND u.suspended = 0 AND u.confirmed = 1
                        ORDER BY u.email ASC";
            }
    ...
    public static function subscribe_user($userid, $peerforum, $context = null, $userrequest = false) {
        ...
        if ($userrequest) {
            $discussionsubscriptions =
                    $DB->get_recordset('peerforum_discussion_subs', array('userid' => $userid, 'peerforum' => $peerforum->id));
            ...
            if (isset(self::$peerforumdiscussioncache[$userid]) &&
                    isset(self::$peerforumdiscussioncache[$userid][$peerforum->id])) {
        ...
        if ($peerforumsubscription = $DB->get_record('peerforum_subscriptions', $sqlparams)) {
            ...
            if ($userrequest) {
                ...
                $DB->delete_records('peerforum_discussion_subs',
                        array('userid' => $userid, 'peerforum' => $peerforum->id,
                                'preference' => self::PEERFORUM_DISCUSSION_UNSUBSCRIBED));
     ...
     */
    public static function get_user_default_subscription($peerforum, $context, $cm, ?int $discussionid) {
        global $USER;
        ...
        if (\mod_peerforum\subscriptions::subscription_disabled($peerforum) && !$manageactivities) {
            ...
            if (isset($discussionid) && self::is_subscribed($USER->id, $peerforum, $discussionid, $cm)) {
                // User is subscribed to the discussion - continue the subscription.
                $discussionsubscribe = true;
            } else if (!isset($discussionid) && \mod_peerforum\subscriptions::is_subscribed($USER->id, $peerforum, null, $cm)) {
