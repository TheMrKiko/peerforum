...
/**
 * A Handler to process replies to peerforum posts.
 *
...
namespace mod_peerforum\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'mod_peerforum');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'mod_peerforum');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        $post = $DB->get_record('peerforum_posts', array('id' => $record->datavalue));
        if (!$post) {
        ...
        $cm = get_fast_modinfo($course->id)->instances['peerforum'][$peerforum->id];
        ...
        if (!$canpost) {
            ...
            throw new \core\message\inbound\processing_failed_exception('messageinboundnopostpeerforum', 'mod_peerforum', $data);
        }
        ...
        if (!\core_availability\info_module::is_user_visible($cm)) {
            $data = new \stdClass();
            $data->peerforum = $peerforum;
            throw new \core\message\inbound\processing_failed_exception('messageinboundpeerforumhidden', 'mod_peerforum', $data);
        }
        ...
        if (!empty($thresholdwarning) && !$thresholdwarning->canpost) {
            ...
            throw new \core\message\inbound\processing_failed_exception('messageinboundthresholdhit', 'mod_peerforum', $data);
        }

        $subject = clean_param($messagedata->envelope->subject, PARAM_TEXT);
        $restring = get_string('re', 'peerforum');
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (empty($peerforum->maxattachments) || $peerforum->maxbytes == 1 ||
                    !has_capability('mod/peerforum:createattachment', $modcontext)) {
                // Attachments are not allowed.
                mtrace("--> User does not have permission to attach files in this peerforum. Rejecting e-mail.");

                $data = new \stdClass();
                $data->peerforum = $peerforum;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundattachmentdisallowed', 'mod_peerforum',
                        $data);
            }

            if ($peerforum->maxattachments < $attachmentcount) {
                // Too many attachments.
                mtrace("--> User attached {$attachmentcount} files when only {$peerforum->maxattachments} where allowed. "
                        . " Rejecting e-mail.");

                $data = new \stdClass();
                $data->peerforum = $peerforum;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilecountexceeded', 'mod_peerforum',
                        $data);
            }
            ...
            if ($peerforum->maxbytes < $filesize) {
                // Too many attachments.
                mtrace("--> User attached {$filesize} bytes of files when only {$peerforum->maxbytes} where allowed. "
                        . "Rejecting e-mail.");
                $data = new \stdClass();
                $data->peerforum = $peerforum;
                $data->maxbytes = display_size($peerforum->maxbytes);
                $data->filesize = display_size($filesize);
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilesizeexceeded', 'mod_peerforum',
                        $data);
            }
        }
        ...
        $addpost->id = peerforum_add_new_post($addpost, true);
        ...
        $event->add_record_snapshot('peerforum_posts', $addpost);
        $event->add_record_snapshot('peerforum_discussions', $discussion);
        $event->trigger();

        // Update completion state.
        $completion = new \completion_info($course);
        if ($completion->is_enabled($cm) && ($peerforum->completionreplies || $peerforum->completionposts)) {
            $completion->update_state($cm, COMPLETION_COMPLETE);

            mtrace("--> Updating completion status for user {$USER->id} in peerforum {$peerforum->id} for post {$addpost->id}.");
        }
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/mod/peerforum/discuss.php', array('d' => $handlerresult->discussion));
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'mod_peerforum', $a);
        $message->html = get_string('postbymailsuccess_html', 'mod_peerforum', $a);
        return $message;
    }
}
