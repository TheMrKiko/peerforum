...
class mod_peerforum_post_form extends moodleform {
    ...
    function definition() {
        ...
        if (isset($peerforum->id) && peerforum_is_forcesubscribed($peerforum)) {
            ...
            $mform->addElement('static', 'subscribemessage', get_string('subscription', 'peerforum'),
                    get_string('everyoneissubscribed', 'peerforum'));
            ...
            $mform->addElement('static', 'subscribemessage', get_string('subscription', 'peerforum'),
                    get_string('disallowsubscribe', 'peerforum'));
            $mform->addElement('hidden', 'subscribe');
            $mform->setType('subscribe', PARAM_INT);
            $mform->addHelpButton('subscribemessage', 'subscription', 'peerforum');
        }
        ...
        if (!empty($peerforum->maxattachments) && $peerforum->maxbytes != 1 &&
                has_capability('mod/peerforum:createattachment', $modcontext)) {  //  1 = No attachments at all
        ...
        if ($groupmode = groups_get_activity_groupmode($cm, $course)) { // hack alert
            ...
            $contextcheck =
                    has_capability('mod/peerforum:movediscussions', $modulecontext) && empty($post->parent) && $groupcount > 1;
            if ($contextcheck) {
