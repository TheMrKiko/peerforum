...
class mod_peerforum_post_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('editor', 'message', get_string('message', 'peerforum'), null,
        ...
        if (!$inpagereply) {
            ...
            if (peerforum_can_create_attachment($peerforum, $modcontext)) {
                $mform->addElement('filemanager', 'attachments', get_string('attachment', 'peerforum'), null,
                        self::attachment_options($peerforum));
                $mform->addHelpButton('attachments', 'attachment', 'peerforum');
            }

            if (!$post->parent && has_capability('mod/peerforum:pindiscussions', $modcontext)) {
                $mform->addElement('checkbox', 'pinned', get_string('discussionpinned', 'peerforum'));
                $mform->addHelpButton('pinned', 'discussionpinned', 'peerforum');
            }
            ...
            if ((empty($post->id) && $canreplyprivately) || (!empty($post) && !empty($post->privatereplyto))) {
                ...
                $mform->addElement('checkbox', 'isprivatereply', get_string('privatereply', 'peerforum'));
                $mform->addHelpButton('isprivatereply', 'privatereply', 'peerforum');
            ...
            if (empty($post->id) && $post->parent && $peerforum->peergradeassessed) {
                if ($peerforum->gradeprofessorpost || !has_capability('mod/peerforum:professorpeergrade', $modcontext)) {
            ...
            if (!empty($CFG->peerforum_enabletimedposts) && !$post->parent &&
                    has_capability('mod/peerforum:viewhiddentimedposts', $coursecontext)) {
                ...
                $mform->addElement('date_time_selector', 'timeend', get_string('displayend', 'peerforum'),
                        array('optional' => true));
            ...
            if (core_tag_tag::is_enabled('mod_peerforum', 'peerforum_posts')) {
                ...
                $mform->addElement('tags', 'tags', get_string('tags'),
                        array('itemtype' => 'peerforum_posts', 'component' => 'mod_peerforum'));
            }
        }
        ...
        if (isset($post->edit)) { // hack alert
            ...
            $submitstring = get_string('posttopeerforum', 'peerforum');
        }
