...
namespace mod_peerforum\search;
...
class post extends \core_search\base_mod {
     ...
     */
    protected $peerforumsdata = array();
    ...
    /**
     * Returns recordset containing required data for indexing peerforum posts.
     *
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        $sql = "SELECT fp.*, f.id AS peerforumid, f.course AS courseid, fd.groupid AS groupid
    ...
    public function get_document($record, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('peerforum', $record->peerforumid, $record->courseid);
    ...
    /**
     * Add the peerforum post attachments.
     *
    ...
    public function attach_files($document) {
        ...
        $cm = $this->get_cm($this->get_module_name(), $post->peerforum, $document->get('courseid'));
        $context = \context_module::instance($cm->id);
     ...
     *
     * @param int $id PeerForum post id
     * @return bool
    ...
    public function check_access($id) {
        ...
        try {
            ...
            $peerforum = $this->get_peerforum($post->peerforum);
            $discussion = $this->get_discussion($post->discussion);
            $cminfo = $this->get_cm('peerforum', $peerforum->id, $peerforum->course);
    ...
    /**
     * Link to the peerforum post discussion
     *
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/peerforum/discuss.php', array('d' => $post->discussion));
    }

    /**
     * Link to the peerforum.
     *
     * @param \core_search\document $doc
     * @return \moodle_url
     */
    public function get_context_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/peerforum/view.php', array('id' => $contextmodule->instanceid));
    }

    /**
     * Returns the specified peerforum post from its internal cache.
     *
     * @param int $postid
     * @return stdClass
     * @throws \dml_missing_record_exception
    ...
    protected function get_post($postid) {
        if (empty($this->postsdata[$postid])) {
            $this->postsdata[$postid] = peerforum_get_post_full($postid);
            if (!$this->postsdata[$postid]) {
                throw new \dml_missing_record_exception('peerforum_posts');
            }
        }
    ...
    /**
     * Returns the specified peerforum checking the internal cache.
     *
     ...
     * @throws \dml_exception
     */
    protected function get_peerforum($peerforumid) {
        global $DB;

        if (empty($this->peerforumsdata[$peerforumid])) {
            $this->peerforumsdata[$peerforumid] = $DB->get_record('peerforum', array('id' => $peerforumid), '*', MUST_EXIST);
        }
        return $this->peerforumsdata[$peerforumid];
    }
    ...
    protected function get_discussion($discussionid) {
        ...
        if (empty($this->discussionsdata[$discussionid])) {
            $this->discussionsdata[$discussionid] = $DB->get_record('peerforum_discussions',
    ...
    /**
     * Changes the context ordering so that the peerforums with most recent discussions are indexed
     * first.
     *
    ...
    protected function get_contexts_to_reindex_extra_sql() {
        return [
                'JOIN {peerforum_discussions} fd ON fd.course = cm.course AND fd.peerforum = cm.instance',
