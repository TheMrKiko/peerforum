...
use mod_peerforum\local\vaults\preprocessors\extract_context as extract_context_preprocessor;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class peerforum extends db_table_vault {
    /** The table for this vault */
    private const TABLE = 'peerforum';
    ...
    protected function generate_get_records_sql(string $wheresql = null, string $sortsql = null, ?int $userid = null): string {
        ...
        $tables .= " JOIN {modules} m ON m.name = 'peerforum'";
    ...
    protected function get_preprocessors(): array {
        return array_merge(
                ...
                [
                        'peerforum' => new extract_record_preprocessor(self::TABLE, $this->get_table_alias()),
    ...
    /**
     * Convert the DB records into peerforum entities.
     *
    ...
    protected function from_db_records(array $results): array {
        ...
        return array_map(function(array $result) use ($entityfactory) {
            ...
            return $entityfactory->get_peerforum_from_stdclass($peerforumrecord, $context, $coursemodule, $course);
        }, $results);
    }

    /**
     * Get the peerforum for the given course module id.
     *
     * @param int $id The course module id
     * @return peerforum_entity|null
     */
    public function get_from_course_module_id(int $id): ?peerforum_entity {
        $records = $this->get_from_course_module_ids([$id]);
        return count($records) ? array_shift($records) : null;
    }

    /**
     * Get the peerforums for the given course module ids
     *
    ...
    /**
     * Get the peerforum entity for the given post id.
     *
     * @param int $id The course module id
     * @return peerforum_entity|null
     */
    public function get_from_post_id(int $id): ?peerforum_entity {
        $alias = $this->get_table_alias();
        $thistable = new dml_table(self::TABLE, $alias, $alias);
        ...
        $tables = "{peerforum_posts} p";
        $tables .= " JOIN {peerforum_discussions} d ON d.id = p.discussion";
        $tables .= ' JOIN {' . self::TABLE . "} {$alias} ON {$alias}.id = d.peerforum";
        $tables .= " JOIN {modules} m ON m.name = 'peerforum'";
