...
class exported_discussion {
    ...
    public function build(
        ...
        $favouriteids = [];
        if ($this->is_favourited($discussion, $peerforum->get_context(), $user)) {
        ...
        $discussionexporter = $this->exporterfactory->get_discussion_exporter(
                $user, $peerforum, $discussion, $groupsbyid, $favouriteids
        );
     ...
     * @param \context_module $peerforumcontext PeerForum context
     ...
     */
    public function is_favourited(discussion_entity $discussion, \context_module $peerforumcontext, \stdClass $user) {
        if (!isloggedin()) {
            return false;
        }
        ...
        return $ufservice->favourite_exists('mod_peerforum', 'discussions', $discussion->get_id(), $peerforumcontext);
    }
