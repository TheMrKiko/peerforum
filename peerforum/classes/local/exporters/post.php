...
class post extends exporter {
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $editurl = $canedit ? $urlfactory->get_edit_post_url_from_post($peerforum, $post) : null;
        ...
        if ($loadcontent) {
            ...
            $subject = $isdeleted ? get_string('peerforumsubjectdeleted', 'peerforum') :
                    get_string('peerforumsubjecthidden', 'peerforum');
            $message =
                    $isdeleted ? get_string('peerforumbodydeleted', 'peerforum') : get_string('peerforumbodyhidden', 'peerforum');
            $timecreated = null;
        }
        ...
        $showwordcount = $peerforum->should_display_word_count();
        if ($showwordcount) {
        ...
        return [
                ...
                'unread' => ($loadcontent && $readreceiptcollection) ? !$readreceiptcollection->has_user_read_post($user, $post) :
                        null,
                'isdeleted' => $isdeleted,
    ...
    protected static function define_related() {
        return [
                ...
                'readreceiptcollection' => 'mod_peerforum\local\entities\post_read_receipt_collection?',
                'urlfactory' => 'mod_peerforum\local\factories\url',
                'peerforum' => 'mod_peerforum\local\entities\peerforum',
                'discussion' => 'mod_peerforum\local\entities\discussion',
                'author' => 'mod_peerforum\local\entities\author',
                'authorcontextid' => 'int?',
                'user' => 'stdClass',
                'context' => 'context',
    ...
    private function get_author_subheading_html(stdClass $exportedauthor, int $timecreated): string {
        ...
        return get_string('bynameondate', 'mod_peerforum', ['name' => $name, 'date' => $date]);
    }
    ...
    private function get_start_time(discussion_entity $discussion, post_entity $post) {
        ...
        if (!empty($CFG->peerforum_enabletimedposts) && ($discussiontime > $posttime)) {
