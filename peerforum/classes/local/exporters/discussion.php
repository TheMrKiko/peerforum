...
class discussion extends exporter {
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $peerforum = $this->related['peerforum'];
        ...
        if ($discussion->has_group()) {
            ...
            if ($group) {
                ...
                if (!$group->hidepicture &&
                        ($url = get_group_picture_url($group, $peerforum->get_course_id(), true))) {
        ...
        $data = [
                ...
                'locked' => $peerforum->is_discussion_locked($discussion),
                'istimelocked' => $peerforum->is_discussion_time_locked($discussion),
                ...
                'userstate' => [
                        'subscribed' => \mod_peerforum\subscriptions::is_subscribed($user->id, $peerforumrecord,
                                $discussion->get_id()),
                ...
                'capabilities' => [
                        ...
                        'manage' => $capabilitymanager->can_manage_peerforum($user),
                        'favourite' => $capabilitymanager->can_favourite_discussion($user)
                    // Defaulting to true until we get capabilities sorted
                ],
                'urls' => [
                        'view' => $urlfactory->get_discussion_view_url_from_discussion($discussion)->out(false),
                        'viewfirstunread' => $viewfirstunreadurl->out(false),
                        'markasread' => $urlfactory->get_mark_discussion_as_read_url_from_discussion($peerforum, $discussion)
                                ->out(false),
    ...
    /**
     * Get the legacy peerforum record from the peerforum entity.
     *
     * @return stdClass
     */
    private function get_peerforum_record() {
        $peerforumdbdatamapper = $this->related['legacydatamapperfactory']->get_peerforum_data_mapper();
        return $peerforumdbdatamapper->to_legacy_object($this->related['peerforum']);
    }
    ...
    protected static function define_related() {
        return [
                ...
                'context' => 'context',
                'peerforum' => 'mod_peerforum\local\entities\peerforum',
