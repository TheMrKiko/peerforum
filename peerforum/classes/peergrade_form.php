 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peergrade_post_form extends moodleform {
    /**
    ...
    function definition() {
        ...
        //$peergrade = $this->_customdata['peergrade'];
        $edit = $this->_customdata['edit'];

        //$mform->addElement('header', 'general', '');//fill in the data depending on page params later using set_data
        $mform->addElement('header', 'configheader', get_string('yourgradeandfeedback', 'peerforum'));

        echo 'classes/grade_form.php';

        //Assign a grade
        //$mform->addElement('text', 'subject', get_string('assignagrade', 'peerforum'), 'size="48"');
        // add dropdown list option
        $options = array();

        $systemcontext = context_system::instance();

        $info_forum = $DB->get_record('peerforum', array('id' => $systemcontext->id));

        $scale = $info_forum->peergradescale;

        if (!empty($scale)) {
            $peergradescale = $scale;

        } else {
            $peergradescale = 4;
        }

        for ($i = $peergradescale; $i >= 0; $i--) {
            $options[$i] = $i;
        }
        $label = get_string('selectpeergrade', 'peerforum');
        $mform->addElement('select', 'selectpeergrade', $label, $options);
        $mform->addRule('selectpeergrade', get_string('required'), 'required', null, 'client');

        //$mform->setDefault('selectgrade', $workshopconfig->gradedecimals);
        ...
        //Give feedback
        $mform->addElement('editor', 'message', get_string('givefeedback', 'peerforum'), null,
        ...
        */
        ...
        if (!empty($CFG->peerforum_enabletimedposts) && !$post->parent &&
                has_capability('mod/peerforum:viewhiddentimedposts', $coursecontext)) { // hack alert
            $mform->addElement('header', 'displayperiod', get_string('displayperiod', 'peerforum'));

            $mform->addElement('date_time_selector', 'timestart', get_string('displaystart', 'peerforum'),
                    array('optional' => true));
        ...
        if ($groupmode = groups_get_activity_groupmode($cm, $course)) {
            ...
            $canselectgroupformove =
                    $groupcount && !empty($post->edit) && has_capability('mod/peerforum:movediscussions', $modcontext);
        ...
        if (isset($post->edit)) { // hack alert
            ...
            $submit_string = get_string('givegrade', 'peerforum');
        }
        ...
        $mform->addElement('hidden', 'peergrade');
        $mform->setType('peergrade', PARAM_INT);
    }
