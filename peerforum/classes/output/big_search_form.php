...
class big_search_form implements renderable, templatable {
    ...
    public function __construct($course) {
        ...
        $this->actionurl = new moodle_url('/mod/peerforum/search.php');

        $peerforumoptions = ['' => get_string('allpeerforums', 'peerforum')] + peerforum_menu_list($course);
        $this->peerforumoptions = array_map(function($option) use ($peerforumoptions) {
            return [
                    'value' => $option,
                    'name' => $peerforumoptions[$option]
            ];
        }, array_keys($peerforumoptions));
    }
    ...
    /**
     * PeerForum ID setter search criteria.
     *
     * @param int $peerforumid The peerforum ID.
     */
    public function set_peerforumid($peerforumid) {
        $this->peerforumid = $peerforumid;
    }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $tagtypestoshow = \core_tag_area::get_showstandard('mod_peerforum', 'peerforum_posts');
        ...
        $data->tagsenabled = \core_tag_tag::is_enabled('mod_peerforum', 'peerforum_posts');
        $namefield = empty($CFG->keeptagnamecase) ? 'name' : 'rawname';
        $tags = $DB->get_records('tag',
                array('isstandard' => 1, 'tagcollid' => \core_tag_area::get_collection('mod_peerforum', 'peerforum_posts')),
        ...
        if ($this->peerforumid && !empty($this->peerforumoptions)) {
            foreach ($this->peerforumoptions as $index => $option) {
                if ($option['value'] == $this->peerforumid) {
                    $this->peerforumoptions[$index]['selected'] = true;
                } else {
                    $this->peerforumoptions[$index]['selected'] = false;
                }
            }
        }
        $data->peerforumoptions = $this->peerforumoptions;
