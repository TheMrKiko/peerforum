...
class mod_peerforum_observer {
    ...
    public static function user_enrolment_deleted(\core\event\user_enrolment_deleted $event) {
        ...
        if ($cp->lastenrol) {
            if (!$peerforums = $DB->get_records('peerforum', array('course' => $cp->courseid), '', 'id')) {
                return;
            }
            list($peerforumselect, $params) = $DB->get_in_or_equal(array_keys($peerforums), SQL_PARAMS_NAMED);
    ...
    public static function course_created(\core\event\course_created $event) {
        ...
        if ($format->supports_news() && !empty($course->newsitems)) {
            require_once($CFG->dirroot . '/mod/peerforum/lib.php');
            // Auto create the announcements peerforum.
            peerforum_get_course_peerforum($event->objectid, 'news');
        }
    }
