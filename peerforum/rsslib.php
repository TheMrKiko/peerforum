...
function peerforum_rss_feed_discussions_sql($peerforum, $cm, $newsince = 0) {
    ...
    $postdata =
            "p.id AS postid, p.subject, p.created as postcreated, p.modified, p.discussion, p.userid, p.message as postmessage, p.messageformat AS postformat, p.messagetrust AS posttrust";
    ...
    $sql = "SELECT $postdata, d.id as discussionid, d.name as discussionname, d.timemodified, d.usermodified, d.groupid,
             ...
             WHERE d.peerforum = {$peerforum->id} AND p.parent = 0 AND p.deleted <> 1
...
function peerforum_rss_feed_contents($peerforum, $sql, $params, $context) {
    ...
    foreach ($recs as $rec) {
        ...
        if ($isdiscussion && !peerforum_user_can_see_discussion($peerforum, $discussion, $context)) {
            ...
            if (peerforum_user_can_see_post($peerforum, $discussion, $post, $USER, $cm, false)) {
                ...
                $item->title = get_string('privacy:request:delete:post:subject', 'mod_peerforum');
                $message = get_string('privacy:request:delete:post:subject', 'mod_peerforum');
                $item->author = get_string('peerforumauthorhidden', 'peerforum');
            }
        } else {
            ...
            if ($isdiscussion && !empty($rec->discussionname)) {
                ...
                $item->title = format_string($peerforum->name . ' ' .
