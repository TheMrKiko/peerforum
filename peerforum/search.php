...
add_to_log($course->id, "peerforum", "search", "search.php?id=$course->id&amp;search=" . urlencode($search), $search);
...
foreach ($posts as $post) {
    ...
    if ($peerforum->type != 'single') {
        ...
        if ($post->parent != 0) {
            $fullsubject .= " -> <a href=\"discuss.php?d=$post->discussion&amp;parent=$post->id\">" .
...
function peerforum_print_big_search_form($course) {
    ...
    echo '<td class="c1"><input type="text" size="35" name="notwords" id="notwords" value="' . s($notwords, true) .
            '" alt="" /></td>';
    echo '</tr>';
    ...
    if ($DB->get_dbfamily() == 'mysql' || $DB->get_dbfamily() == 'postgres') {
        ...
        echo '<td class="c1"><input type="text" size="35" name="fullwords" id="fullwords" value="' . s($fullwords, true) .
                '" alt="" /></td>';
        echo '</tr>';
    }
    ...
    echo '<input name="timefromrestrict" type="checkbox" value="1" alt="' . get_string('searchdatefrom', 'peerforum') .
    ...
    echo '<input name="timetorestrict" type="checkbox" value="1" alt="' . get_string('searchdateto', 'peerforum') .
    ...
    echo '<td class="c1">';
    echo html_writer::select(peerforum_menu_list($course), 'peerforumid', '',
            array('' => get_string('allpeerforums', 'peerforum')));
    echo '</td>';
    echo '</tr>';
