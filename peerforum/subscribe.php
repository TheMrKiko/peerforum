...
if (is_null($mode) and !is_enrolled($context, $USER, '', true)) {   // Guests and visitors can't subscribe - only enrolled
    ...
    if (isguestuser()) {
        ...
        redirect(
                new moodle_url('/mod/peerforum/view.php', array('f' => $id)),
...
if (!is_null($mode) and has_capability('mod/peerforum:managesubscriptions', $context)) {
    ...
    switch ($mode) {
        case PEERFORUM_CHOOSESUBSCRIBE : // 0
            ...
            redirect(
                    $returnto,
                    get_string('everyonecannowchoose', 'peerforum'),
                    null,
        ...
        case PEERFORUM_FORCESUBSCRIBE : // 1
            ...
            redirect(
                    $returnto,
                    get_string('everyoneisnowsubscribed', 'peerforum'),
                    null,
                    \core\output\notification::NOTIFY_SUCCESS
            );
            break;
        ...
        case PEERFORUM_DISALLOWSUBSCRIBE : // 3
            ...
            redirect(
                    $returnto,
                    get_string('noonecansubscribenow', 'peerforum'),
                    null,
                    \core\output\notification::NOTIFY_SUCCESS
            );
            break;
...
if ($issubscribed) {
    ...
    if ($discussionid === null) {
        if (\mod_peerforum\subscriptions::unsubscribe_user($user->id, $peerforum, $context, true)) {
            redirect(
                    $returnto,
                    get_string('nownotsubscribed', 'peerforum', $info),
                    null,
                    \core\output\notification::NOTIFY_SUCCESS
            );
        } else {
        ...
        if (\mod_peerforum\subscriptions::unsubscribe_user_from_discussion($user->id, $discussion, $context)) {
            ...
            redirect(
                    $returnto,
                    get_string('discussionnownotsubscribed', 'peerforum', $info),
                    null,
                    \core\output\notification::NOTIFY_SUCCESS
            );
        } else {
    ...
    if (\mod_peerforum\subscriptions::subscription_disabled($peerforum) &&
    ...
    if ($discussionid == null) {
        ...
        redirect(
                $returnto,
                get_string('nowsubscribed', 'peerforum', $info),
                null,
                \core\output\notification::NOTIFY_SUCCESS
        );
    } else {
        ...
        redirect(
                $returnto,
                get_string('discussionnowsubscribed', 'peerforum', $info),
                null,
                \core\output\notification::NOTIFY_SUCCESS
        );
    }
