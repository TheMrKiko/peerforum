...
if (data_submitted() and $confirm and confirm_sesskey()) {
    $peerforums = peerforum_get_optional_subscribed_peerforums();

    foreach ($peerforums as $peerforum) {
        peerforum_unsubscribe($USER->id, $peerforum->id);
    }
    ...
    $a = count(peerforum_get_optional_subscribed_peerforums());

    if ($a) {
        $msg = get_string('unsubscribeallconfirm', 'peerforum', $a);
