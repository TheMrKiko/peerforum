...
/**
 * Provides rendering functionality for the peerforum summary report subplugin.
 *
...
use peerforumreport_summary\summary_table;

/**
 * Renderer for the peerforum summary report.
 *
 * @copyright  2019 Michael Hawkins <michaelh@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class peerforumreport_summary_renderer extends plugin_renderer_base {

    /**
     * Render the filters available for the peerforum summary report.
     *
    ...
    public function render_filters_form(stdClass $course, array $cms, moodle_url $actionurl, array $filters = []): string {
        $renderable = new \peerforumreport_summary\output\filters($course, $cms, $actionurl, $filters);
        $templatecontext = $renderable->export_for_template($this);

        return $this->render_from_template('peerforumreport_summary/filters', $templatecontext);
    }
    ...
    public function render_summary_table(summary_table $table): string {
        ...
        return $this->render_from_template('peerforumreport_summary/report', ['tablehtml' => $tablehtml]);
    }
}
