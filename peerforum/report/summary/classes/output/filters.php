...
/**
 * PeerForum summary report filters renderable.
 *
...
namespace peerforumreport_summary\output;
...
class filters implements renderable, templatable {
     ...
     * within that group in the peerforum.
    ...
    protected function prepare_groups_data(array $groupsdata): void {
        ...
        // Check if any peerforum gives the user access to all groups and no groups.
        foreach ($this->cms as $cm) {
            ...
            if (!$allgroups) {
                ...
                // Fetch for the current cm's peerforum.
        ...
        if ($groupsselectedcount > 0 && $groupsselectedcount < count($groupsavailable)) {
            ...
            $discussionswhere = "course = :courseid AND peerforum {$peerforumidin} AND groupid {$groupidin}";
            $discussionsparams = ['courseid' => $this->courseid];
            $discussionsparams += $peerforumidparams + $groupidparams;

            $discussionids =
                    $DB->get_fieldset_select('peerforum_discussions', 'DISTINCT id', $discussionswhere, $discussionsparams);
    ...
    private function prepare_dates_data(array $datefromdata, array $datetodata): void {
        ...
        if ($fromenabled && $toenabled) {
            ...
            $this->datesbuttontext = get_string('filter:datesfromto', 'peerforumreport_summary', $datestrings);
        } else if ($fromenabled) {
            $this->datesbuttontext = get_string('filter:datesfrom', 'peerforumreport_summary', $fromdatestring);
        } else if ($toenabled) {
            $this->datesbuttontext = get_string('filter:datesto', 'peerforumreport_summary', $todatestring);
        } else {
            $this->datesbuttontext = get_string('filter:datesname', 'peerforumreport_summary');
        }
    }
    ...
    public function export_for_template(renderer_base $renderer): stdClass {
        ...
        if (!empty($this->groupsavailable)) {
            ...
            if (count($this->groupsavailable) <= $groupscount) {
                $output->filtergroupsname = get_string('filter:groupscountall', 'peerforumreport_summary');
            } else if (!empty($this->groupsselected)) {
                $output->filtergroupsname = get_string('filter:groupscountnumber', 'peerforumreport_summary', $groupscount);
            } else {
                $output->filtergroupsname = get_string('filter:groupsname', 'peerforumreport_summary');
            }
        ...
        $datesform = new peerforumreport_summary\form\dates_filter_form();
