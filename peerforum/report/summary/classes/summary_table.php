...
/**
 * The class for displaying the peerforum report table.
 *
...
namespace peerforumreport_summary;
...
class summary_table extends table_sql {
    ...
    /** PeerForum filter type */
    const FILTER_PEERFORUM = 1;
    ...
    /** @var int The course ID containing the peerforum(s) being reported on. */
    protected $courseid;

    /** @var bool True if reporting on all peerforums in course user has access to, false if reporting on a single peerforum */
    protected $iscoursereport = false;

    /** @var bool True if user has access to all peerforums in the course (and is running course report), otherwise false. */
    protected $accessallpeerforums = false;

    /** @var \stdClass The course module object(s) of the peerforum(s) being reported on. */
    protected $cms = [];
    ...
    /**
     * @var array of \context objects for the peerforums included in the report.
     */
    protected $peerforumcontexts = [];

    /**
     * @var context_course|context_module The context where the report is being run (either a specific peerforum or the course).
     */
    ...
    /**
     * PeerForum report table constructor.
     *
     * @param int $courseid The ID of the course the peerforum(s) exist within.
     ...
     * @param bool $accessallpeerforums If user is running a course level report, do they have access to all peerforums in the
     *         course?
     */
    public function __construct(int $courseid, array $filters, bool $allowbulkoperations,
            bool $canseeprivatereplies, int $perpage, bool $canexport, bool $iscoursereport, bool $accessallpeerforums) {
        global $OUTPUT;

        $uniqueid = $courseid . ($iscoursereport ? '' : '_' . $filters['peerforums'][0]);
        parent::__construct("summaryreport_{$uniqueid}");

        $this->courseid = $courseid;
        $this->iscoursereport = $iscoursereport;
        $this->accessallpeerforums = $accessallpeerforums;
        $this->allowbulkoperations = $allowbulkoperations;
        $this->canseeprivatereplies = $canseeprivatereplies;
        $this->perpage = $perpage;

        $this->set_peerforum_properties($filters['peerforums']);
        ...
        $columnheaders += [
                ...
                'postcount' => get_string('postcount', 'peerforumreport_summary'),
                'replycount' => get_string('replycount', 'peerforumreport_summary'),
                'attachmentcount' => get_string('attachmentcount', 'peerforumreport_summary'),
        ];

        $this->logreader = $this->get_internal_log_reader();
        if ($this->logreader) {
            $columnheaders['viewcount'] = get_string('viewcount', 'peerforumreport_summary');
        }

        if ($this->show_word_char_counts()) {
            $columnheaders['wordcount'] = get_string('wordcount', 'peerforumreport_summary');
            $columnheaders['charcount'] = get_string('charcount', 'peerforumreport_summary');
        }

        $columnheaders['earliestpost'] = get_string('earliestpost', 'peerforumreport_summary');
        $columnheaders['latestpost'] = get_string('latestpost', 'peerforumreport_summary');

        if ($canexport) {
            $columnheaders['export'] = get_string('exportposts', 'peerforumreport_summary');
        }
    ...
    /**
     * Sets properties that are determined by peerforum filter values.
     *
     * @param array $peerforumids The peerforum IDs passed in by the filter.
     * @return void
     */
    protected function set_peerforum_properties(array $peerforumids): void {
        global $USER;

        // Course context if reporting on all peerforums in the course the user has access to.
        if ($this->iscoursereport) {
            $this->userfieldscontext = \context_course::instance($this->courseid);
        }

        foreach ($peerforumids as $peerforumid) {
            $cm = get_coursemodule_from_instance('peerforum', $peerforumid, $this->courseid);
            $this->cms[] = $cm;
            $this->peerforumcontexts[$cm->id] = \context_module::instance($cm->id);

            // Set peerforum context if not reporting on course.
            if (!isset($this->userfieldscontext)) {
                $this->userfieldscontext = $this->peerforumcontexts[$cm->id];
            }

            // Only show own summary unless they have permission to view all in every peerforum being reported.
            if (empty($this->userid) && !has_capability('peerforumreport/summary:viewall', $this->peerforumcontexts[$cm->id])) {
                $this->userid = $USER->id;
            }
        }
    }
    ...
    protected function get_filter_name(int $filtertype): string {
        $filternames = [
                self::FILTER_PEERFORUM => 'PeerForum',
    ...
    public function col_export(\stdClass $data): string {
        ...
        $params = [
                'id' => $this->cms[0]->instance, // PeerForum id.
        ...
        $buttoncontext = [
                'url' => new \moodle_url('/mod/peerforum/export.php', $params),
                'label' => get_string('exportpostslabel', 'peerforumreport_summary', fullname($data)),
        ];

        return $OUTPUT->render_from_template('peerforumreport_summary/export_link_button', $buttoncontext);
    }
    ...
    public function add_filter(int $filtertype, array $values = []): void {
        ...
        switch ($filtertype) {
            case self::FILTER_PEERFORUM:
                // Requires at least one peerforum ID.
                if (empty($values)) {
                    $paramcounterror = true;
                } else {
                    // No select fields required - displayed in title.
                    // No extra joins required, peerforum is already joined.
                    list($peerforumidin, $peerforumidparams) = $DB->get_in_or_equal($values, SQL_PARAMS_NAMED);
                    $this->sql->filterwhere .= " AND f.id {$peerforumidin}";
                    $this->sql->params += $peerforumidparams;
                }
    ...
    protected function define_table_configs(): void {
        ...
        $this->set_attribute('id', 'peerforumreport_summary_table');
    ...
    protected function define_base_sql(): void {
        ...
        $this->sql->basefromjoins = '    {enrol} e
                                    ...
                                    JOIN {peerforum} f ON f.course = e.courseid
    ...
    protected function apply_filters(array $filters): void {
        // Apply the peerforums filter if not reporting on every peerforum in a course.
        if (!$this->accessallpeerforums) {
            $this->add_filter(self::FILTER_PEERFORUM, $filters['peerforums']);
        }
    ...
    protected function fill_log_summary_temp_table() {
        ...
        foreach ($this->peerforumcontexts as $peerforumcontext) {
            $contextids[] = $peerforumcontext->id;
        }
    ...
    protected function get_filter_groups(array $groups): array {
        ...
        foreach ($this->cms as $cm) {
            ...
            if (!$allgroups) {
                ...
                // If no groups mode enabled on the peerforum, nothing to prepare.
                if (!in_array($groupmode, [VISIBLEGROUPS, SEPARATEGROUPS])) {
                    continue;
                }

                $aag = has_capability('moodle/site:accessallgroups', $this->peerforumcontexts[$cm->id]);
    ...
    protected function show_word_char_counts(): bool {
        ...
        if (is_null($this->showwordcharcounts)) {
            ...
            foreach ($this->cms as $cm) {
                $peerforumids[] = $cm->instance;
            }

            list($peerforumidin, $peerforumidparams) = $DB->get_in_or_equal($peerforumids, SQL_PARAMS_NAMED);

            // This should be really fast.
            $sql = "SELECT 'x'
                      FROM {peerforum_posts} fp
                      JOIN {peerforum_discussions} fd ON fd.id = fp.discussion
                     WHERE fd.peerforum {$peerforumidin} AND (fp.wordcount IS NULL OR fp.charcount IS NULL)";

            if ($DB->record_exists_sql($sql, $peerforumidparams)) {
    ...
    public function wrap_html_finish(): void {
        ...
        echo $OUTPUT->render_from_template('peerforumreport_summary/bulk_action_menu', $data);
    }
}
