...
if (!empty($peerforum)) {      // User is starting a new discussion in a peerforum
    ...
    if ($prunemform->is_cancelled()) {
        ...
        $PAGE->navbar->add(format_string($post->subject, true),
                new moodle_url('/mod/peerforum/discuss.php', array('d' => $discussion->id)));
...
$currenttext = file_prepare_draft_area($draftid_editor, $modcontext->id, 'mod_peerforum', 'post', $postid,
        mod_peerforum_post_form::editor_options($modcontext, $postid), $post->message);
...
if ($mform_post->is_cancelled()) {
    ...
    if ($fromform->edit) {           // Updating a post
        ...
        // MDL-11818
        if (($peerforum->type == 'single') &&
                ($updatepost->parent == '0')) { // updating first post of single discussion type -> updating peerforum intro
