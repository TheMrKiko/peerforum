...
$pageparams = array('reply' => $reply, 'peerforum' => $peerforum, 'edit' => $edit);
...
if (!isloggedin() or isguestuser()) {
    ...
    if (!empty($peerforum)) {
        ...
        $peerforumentity = $peerforumvault->get_from_post_id($reply);
        if (empty($peerforumentity)) {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->confirm(get_string('noguestpost', 'peerforum') . '<br /><br />' . get_string('liketologin'), get_login_url(),
            $referer);
    echo $OUTPUT->footer();
    exit;
}
...
if (!empty($peerforum)) {
    ...
    if ($peerforumentity->is_in_group_mode() && null === $groupid) {
    ...
    if (!$cm->visible and !has_capability('moodle/course:viewhiddenactivities', $modcontext)) {
        redirect(
                $urlfactory->get_course_url_from_peerforum($peerforumentity),
    ...
    if (!$capabilitymanager->can_reply_to_post($USER, $discussionentity, $parententity)) {
        if (!isguestuser()) {
            ...
            // The peerforum has been locked. Just redirect back to the discussion page.
    ...
    if ($parententity->is_private_reply()) {
        print_error('cannotreplytoprivatereply', 'peerforum');
    }
    ...
    $post->groupid = ($discussion->groupid == -1) ? 0 : $discussion->groupid;
    if ($postentity->has_parent()) {
        $canreplyprivately = peerforum_user_can_reply_privately($modcontext, $parent);
    }
    ...
    if (!empty($confirm) && confirm_sesskey()) {
        ...
        try {
            ...
            if (!$postentity->has_parent()) {
                ...
                if ($peerforumentity->get_type() == 'single') {
                    // Single discussion peerforums are an exception.
                    // We show the peerforum itself since it only has one discussion thread.
    ...
    if (!$postentity->has_parent()) {
        redirect(
                $urlfactory->get_discussion_view_url_from_discussion($discussionentity),
                get_string('alreadyfirstpost', 'peerforum'),
                null,
                \core\output\notification::NOTIFY_ERROR
        );
    }
    if (!$capabilitymanager->can_split_post($USER, $discussionentity, $postentity)) {
        redirect(
                $urlfactory->get_discussion_view_url_from_discussion($discussionentity),
                get_string('cannotsplit', 'peerforum'),
                null,
                \core\output\notification::NOTIFY_ERROR
        );
    }
    ...
    if ($prunemform->is_cancelled()) {
        redirect(peerforum_go_back_to($urlfactory->get_discussion_view_url_from_discussion($discussionentity)));
        ...
        redirect(
                peerforum_go_back_to($urlfactory->get_discussion_view_url_from_post($postentity)),
                get_string('discussionsplit', 'peerforum'),
                null,
                \core\output\notification::NOTIFY_SUCCESS
        );
    } else {
        ...
        $PAGE->navbar->add($subjectstr, new moodle_url('/mod/peerforum/discuss.php', array('d' => $discussion->id)));
        ...
        $peerforumentity = $entityfactory->get_peerforum_from_stdclass($peerforum, $modcontext, $cm, $course);
        $rendererfactory = mod_peerforum\local\container::get_renderer_factory();
        $postsrenderer = $rendererfactory->get_single_discussion_posts_renderer(null, true);
        echo $postsrenderer->render($USER, [$peerforumentity], [$discussionentity], [$postentity]);
    }
...
$mformpost = new mod_peerforum_post_form('post.php', [
        'course' => $course,
        'cm' => $cm,
        ...
        'peerforum' => $peerforum,
        'post' => $post,
        'subscribe' => \mod_peerforum\subscriptions::is_subscribed($USER->id, $peerforum, null, $cm),
        'thresholdwarning' => $thresholdwarning,
        'edit' => $edit,
        'canreplyprivately' => $canreplyprivately,
], 'post', '', array('id' => 'mformpeerforum'));

$draftitemid = file_get_submitted_draft_itemid('attachments');
$postid = empty($post->id) ? null : $post->id;
$attachoptions = mod_peerforum_post_form::attachment_options($peerforum);
file_prepare_draft_area($draftitemid, $modcontext->id, 'mod_peerforum', 'attachment', $postid, $attachoptions);
...
$editoropts = mod_peerforum_post_form::editor_options($modcontext, $postid);
$currenttext =
        file_prepare_draft_area($draftideditor, $modcontext->id, 'mod_peerforum', 'post', $postid, $editoropts, $post->message);
$discussionid = isset($discussion) ? $discussion->id : null;
$discussionsubscribe = \mod_peerforum\subscriptions::get_user_default_subscription($peerforum, $coursecontext, $cm, $discussionid);
...
if ($mformpost->is_cancelled()) {
    if (!isset($discussion->id) || $peerforum->type === 'single') {
        // Single peerforums don't have a discussion page.
    ...
    if ($fromform->edit) {
        ...
        if (!$postentity->has_parent()) {
            if ($capabilitymanager->can_pin_discussions($USER)) {
                ...
                $fromform->pinned = !empty($fromform->pinned) ? PEERFORUM_DISCUSSION_PINNED : PEERFORUM_DISCUSSION_UNPINNED;
            } else {
        ...
        $subscribemessage = peerforum_post_subscription($fromform, $peerforum, $discussion);
        if ('single' == $peerforumentity->get_type()) {
            // Single discussion peerforums are an exception.
            // We show the peerforum itself since it only has one discussion thread.
            $discussionurl = $urlfactory->get_peerforum_view_url_from_peerforum($peerforumentity);
        } else {
        ...
        if ($fromform->id = peerforum_add_new_post($addpost, $mformpost)) {
        ...
        $redirectto = new moodle_url('/mod/peerforum/view.php', array('f' => $fromform->peerforum));
        ...
        redirect(
                peerforum_go_back_to($redirectto->out()),
                $message . $subscribemessage,
                null,
                \core\output\notification::NOTIFY_SUCCESS
        );
...
} else if ('news' == $peerforumentity->get_type()) {
    $titlesubject = get_string("addanewtopic", "peerforum");
} else {
...
if (!empty($discussionentity) && 'qanda' == $peerforumentity->get_type()) {
    $displaywarning = $capabilitymanager->must_post_before_viewing_discussion($USER, $discussionentity);
    $displaywarning =
            $displaywarning && !peerforum_user_has_posted($peerforumentity->get_id(), $discussionentity->get_id(), $USER->id);
    if ($displaywarning) {
...
if (!empty($parententity)) {
    ...
    if (empty($post->edit)) {
        if ('qanda' != $peerforumentity->get_type() || peerforum_user_can_see_discussion($peerforum, $discussion, $modcontext)) {
            $replies = $postvault->get_replies_to_post(
                    $USER,
                    $parententity,
    ...
    $rendererfactory = mod_peerforum\local\container::get_renderer_factory();
    $postsrenderer = $rendererfactory->get_single_discussion_posts_renderer(PEERFORUM_MODE_THREADED, true);
    echo $postsrenderer->render($USER, [$peerforumentity], [$discussionentity], $postentities);
} else {
...
if (!empty($postentity)) {
    $data = (object) [
            'tags' => core_tag_tag::get_item_tags_array('mod_peerforum', 'peerforum_posts', $postentity->get_id())
    ];
