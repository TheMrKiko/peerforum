...
class mod_peerforum_renderer extends plugin_renderer_base {
    ...
    public function render_peergrade(peergrade $peergrade) {
        ...
        //ir buscar peergraders deste post
        $post_info = $DB->get_record("peerforum_posts", array('id' => $post_topeergrade));
        ...
        //Average of peer grades: X (X)
        if ($isstudent && $peerforum->showpeergrades || !$isstudent) { /*&& $canshowafterrating*/
        ...
        if (!$isstudent) {
            ...
            if ($peerforum->showdetails == 1) {
                ...
                $students_assigned_rmv = get_students_assigned($courseid, $post_topeergrade);
    ...
    public function subscriber_overview($users, $peerforum, $course) {
        ...
        if (!$users || !is_array($users) || count($users) === 0) {
            ...
            $output .= $this->output->heading(get_string("subscribersto", "peerforum",
                    "'" . format_string($peerforum->name) . "'"));
            $table = new html_table();
            $table->cellpadding = 5;
            $table->cellspacing = 5;
