...
import Discussion from 'mod_peerforum/discussion';
import InPageReply from 'mod_peerforum/inpage_reply';
import LockToggle from 'mod_peerforum/lock_toggle';
import FavouriteToggle from 'mod_peerforum/favourite_toggle';
import Pin from 'mod_peerforum/pin_toggle';
import Selectors from 'mod_peerforum/selectors';
import Subscribe from 'mod_peerforum/subscription_toggle';
...
const getPostContentContainer = (postContainer) => {
    return postContainer.children().not(Selectors.post.repliesContainer).find(Selectors.post.peerforumCoreContent);
};
...
const renderInPageReplyTemplate = (additionalTemplateContext, button, postContainer) => {
    ...
    const currentSubject = postContentContainer.find(Selectors.post.peerforumSubject).text();
    ...
    return Templates.render('mod_peerforum/inpage_reply_v2', context);
};
