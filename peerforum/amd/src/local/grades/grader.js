 ...
 *
 * @module     mod_peerforum/local/grades/grader
...
import {createLayout as createFullScreenWindow} from 'mod_peerforum/local/layout/fullscreen';
...
const templateNames = {
    grader: {
        app: 'mod_peerforum/local/grades/grader',
        gradingPanel: {
            error: 'mod_peerforum/local/grades/local/grader/gradingpanel/error',
        },
        searchResults: 'mod_peerforum/local/grades/local/grader/user_picker/user_search',
        status: 'mod_peerforum/local/grades/local/grader/status',
    },
};
...
const getSaveUserGradeFunction = (root, setGradeForUser) => {
    return async (user) => {
        try {
            ...
            if (result.success) {
                addToast(await getString('grades:gradesavedfor', 'mod_peerforum', user));
            }
            if (result.failed) {
...
const displayGradingError = async (root, user, err) => {
    const [
        ...
        await getString('grades:gradesavefailed', 'mod_peerforum', {error: err.message, ...user}),
    ]);
...
export const view = async (getGradeForUser, userid, moduleName, {
    ...
    const {html, js} = await Templates.renderForPromise('mod_peerforum/local/grades/view_grade', userGrade);
