...
function peerforum_cron() {
    ...
    // The main renderers.
    $htmlout = $PAGE->get_renderer('mod_peerforum', 'email', 'htmlemail');
    $textout = $PAGE->get_renderer('mod_peerforum', 'email', 'textemail');
    $htmldigestfullout = $PAGE->get_renderer('mod_peerforum', 'emaildigestfull', 'htmlemail');
    $textdigestfullout = $PAGE->get_renderer('mod_peerforum', 'emaildigestfull', 'textemail');
    $htmldigestbasicout = $PAGE->get_renderer('mod_peerforum', 'emaildigestbasic', 'htmlemail');
    $textdigestbasicout = $PAGE->get_renderer('mod_peerforum', 'emaildigestbasic', 'textemail');
    ...
    $messageinboundgenerator->set_handler('\mod_peerforum\message\inbound\reply_handler');
        ...
        foreach ($posts as $pid => $post) {
            ...
            if (!isset($discussions[$discussionid])) {
                if ($discussion = $DB->get_record('peerforum_discussions', array('id' => $post->discussion))) {
                    ...
                    \mod_peerforum\subscriptions::fill_subscription_cache($discussion->peerforum);
                    \mod_peerforum\subscriptions::fill_discussion_subscription_cache($discussion->peerforum);
            ...
            if (!isset($subscribedusers[$peerforumid])) {
                ...
                if ($subusers =
                        \mod_peerforum\subscriptions::fetch_subscribed_users($peerforums[$peerforumid], 0, $modcontext, 'u.*',
                                true)) {
    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                // This does not necessarily mean that the user is subscribed to the peerforum or to the discussion though.
                if (!isset($subscribedusers[$peerforum->id][$userto->id])) {
                    // The user does not subscribe to this peerforum.
                    continue;
                }

                if (!\mod_peerforum\subscriptions::is_subscribed($userto->id, $peerforum, $post->discussion,
                        $coursemodules[$peerforum->id])) {
                    // The user does not subscribe to this peerforum, or to this specific discussion.
                    continue;
                }

                if ($subscriptiontime = \mod_peerforum\subscriptions::fetch_discussion_subscription($peerforum->id, $userto->id)) {
                ...
                if (!isset($userto->viewfullnames[$peerforum->id])) {
                    ...
                    $data->viewfullnames = $userto->viewfullnames[$peerforum->id];
                }

                $a = new stdClass();
                $a->courseshortname = $data->get_coursename();
                $a->peerforumname = $cleanpeerforumname;
                $a->subject = $data->get_subject();
                $postsubject = html_to_text(get_string('postmailsubject', 'peerforum', $a), 0);

                $rootid = peerforum_get_email_message_id($discussion->firstpost, $userto->id, $hostname);

                if ($post->parent) {
                    // This post is a reply, so add reply header (RFC 2822).
                    $parentid = peerforum_get_email_message_id($post->parent, $userto->id, $hostname);
                ...
                $threadtopic = html_to_text(get_string('postmailsubject', 'peerforum', $a), 0);
                ...
                if (!empty($replyaddress)) {
                    ...
                    $htmlfooter = html_writer::tag('p', get_string('replytopostbyemail', 'mod_peerforum'));
                ...
                $contexturl = new moodle_url('/mod/peerforum/discuss.php', array('d' => $discussion->id), 'p' . $post->id);
    ...
    if ($CFG->digestmailtimelast < $digesttime and $timenow > $digesttime) {
        ...
        if ($digestposts_rs->valid()) {
            ...
            foreach ($userdiscussions as $userid => $thesediscussions) {
                ...
                $headerdata->userprefs = $CFG->wwwroot . '/user/peerforum.php?id=' . $userid . '&amp;course=' . $site->id;
                ...
                foreach ($thesediscussions as $discussionid) {
                    ...
                    $canunsubscribe = !\mod_peerforum\subscriptions::is_forcesubscribed($peerforum);
                    $canreply = $userto->canpost[$discussion->id];
                    $shortname = format_string($course->shortname, true, array('context' => context_course::instance($course->id)));
                ...
                if (!$mailresult) {
                    mtrace("ERROR: mod/peerforum/cron.php: Could not send out digest mail to user $userto->id " .
                            "($userto->email)... not trying again.");
                } else {
                    mtrace("success.");
                    $usermailcount++;
...
function peerforum_print_overview($courses, &$htmlarray) {
    ...
    $sql = "SELECT d.id, d.peerforum, d.course, d.groupid, COUNT(*) as count "
            . 'FROM {peerforum_discussions} d '
            . 'JOIN {peerforum_posts} p ON p.discussion = d.id '
            . "WHERE ($coursessql) "
            . 'AND p.userid != ? '
            . 'AND (d.timestart <= ? AND (d.timeend = 0 OR d.timeend > ?)) '
            . 'GROUP BY d.id, d.peerforum, d.course, d.groupid '
            . 'ORDER BY d.course, d.peerforum';
    $params[] = time();
    $params[] = time();
    ...
    if (count($trackingpeerforums) > 0) {
        ...
        $sql .= 'GROUP BY d.peerforum,d.course';
        $params[] = $cutoffdate;
        $params[] = time();
        $params[] = time();
...
function peerforum_count_discussion_replies($peerforumid, $peerforumsort = "", $limit = -1, $page = -1, $perpage = 0) {
    ...
    if (($limitfrom == 0 and $limitnum == 0) or $peerforumsort == "") {
        ...
        return $DB->get_records_sql($sql, array($peerforumid), $limitfrom, $limitnum);
    }
}
            ...
            if ($peerforumtracked) {
                if ($post->unread > 0) {
                    ...
                    echo '<a title="' . $strmarkalldread . '" href="' . $CFG->wwwroot . '/mod/peerforum/markposts.php?f=' .
                            $peerforum->id . '&amp;d=' . $post->discussion . '&amp;mark=read&amp;returnpage=view.php&amp;sesskey=' .
                            sesskey() . '">' .
 ...
 * @return  bool
 */
function peerforum_update_post($newpost, $mform, $unused = null) {
    global $DB, $USER;

    $post = $DB->get_record('peerforum_posts', array('id' => $newpost->id));
    $discussion = $DB->get_record('peerforum_discussions', array('id' => $post->discussion));
    $peerforum = $DB->get_record('peerforum', array('id' => $discussion->peerforum));
    $cm = get_coursemodule_from_instance('peerforum', $peerforum->id);
    $context = context_module::instance($cm->id);
    ...
    $post->message = file_save_draft_area_files($newpost->itemid, $context->id, 'mod_peerforum', 'post', $post->id,
...
function peerforum_user_can_post_discussion($peerforum, $currentgroup = null, $unused = -1, $cm = null, $context = null) {
    ...
    if ($peerforum->type == 'eachuser') {
        if (peerforum_user_has_posted_discussion($peerforum->id, $USER->id, $currentgroup)) {
            return false;
        }
    }
            ...
            if ($cantrack) {
                ...
                if ($peerforumtracked) {
                    echo '<a title="' . get_string('markallread', 'peerforum') .
                            ...
                            $peerforum->id . '&amp;mark=read&amp;returnpage=view.php&amp;sesskey=' . sesskey() . '">' .
