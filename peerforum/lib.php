...
function verify_time_to_edit($itemid, $userid) {
    ...
    if (!empty($peergrade_timecreated_db)) {
        ...
        $delta = time() - $peergrade_timecreated;
...
function assign_peergraders($user, $postid, $courseid, $peerforumid) {
    ...
    $this_context = context_course::instance($courseid); // global $COURSE
    $isstudent = current(get_user_roles($this_context, $postauthor))->shortname == 'student' ? true : false;

    //Check if parent post id == 0
    adjust_database();
    $peerforum = $DB->get_record('peerforum', array('id' => $peerforumid));
    //   $postparent = $DB->get_record('peerforum_posts', array('id' => $itemid))->parent;
    //if post is a reply to some other post and not to the main one, auto assign
...
function verify_post_expired($postid, $peerforum, $userid, $courseid) {
    ...
    if (!empty($time_assign)) {
        ...
        if ($time_current <= $time_finish && $time_interval->invert > 0) {
            $data->post_expired = false;
            $data->time_interval = $time_interval;
            $data->time_current = $time_current;
...
function update_peergrader_of_peergrade_users($peers_info, $itemid) {
    ...
    $data->numpoststopeergrade = $numgrade;
...
function adjust_database() {
    ...
    foreach ($posts_users as $key => $value) {
        ...
        $numpoststopeergrade = $posts_users[$key]->numpoststopeergrade;
        ...
        if ($num_poststopeergrade != $numpoststopeergrade) {
            $data = new stdClass();
            $data->id = $posts_users[$key]->id;
            $data->numpoststopeergrade = $num_poststopeergrade;
            $DB->update_record('peerforum_peergrade_users', $data);
        }
...
function get_students_can_be_assigned_id($postid) {
    ...
    /*$already_assigned = $post_info->assigned_users;
     $already_assigned = explode(';', $already_assigned);


     $duplicates = var_dump(array_intersect( $not_assigned, $already_assigned));
     if (!empty($duplicates)){
       foreach ($duplicates as $key => $value){
         unset($not_assigned[$var]);
       }
     }*/
...
function update_peer_peergrade_users_new($peer_info, $itemid) {
    global $DB;
...
function peerforum_update_instance($peerforum, $mform) {
    ...
    $subjects_info = $DB->get_records("peerforum_peergrade_subject", array('courseid' => $COURSE->id));

    //All subjects registed on the "peergrade_subjects" database ONLY!
    $subject_names = get_all_subjects($subjects_info);

    //Check what if random distribution was previously activated
    $subjects_type1 = $DB->get_records("peerforum_peergrade_subject", array('type' => 1, 'courseid' => $COURSE->id));

    if (count($subjects_type1) == count($subjects_info)) {
        $current_rand_dist = 1;
    } else {
        $current_rand_dist = 0;
    }
    ...
    if (!empty($fromform)) {
        //TODO: Does not have in consideration if previously random distribution was set or not.
        if ($fromform->threaded_grading && !empty($fromform->topicstoattribute)) {

            //Topics + type
            $topics = $fromform->topicstoattribute;
            $topics_choosen = array_id_to_name($topics);
            $type = $fromform->typestoattribute;

            foreach ($topics_choosen as $i => $value) {
                if (!in_array($topics_choosen[$i], $subject_names)) {
                    $data = new stdClass();
                    $data->name = (string) $topics_choosen[$i];
                    $data->type = $type + 1;
                    $data->courseid = $COURSE->id;
                    $data->peerforumid = $peerforum->id;
                    $DB->insert_record('peerforum_peergrade_subject', $data);

                } else {
                    $thisvalue = $DB->get_record("peerforum_peergrade_subject",
                            array('name' => (string) $topics_choosen[$i], 'courseid' => $COURSE->id));
                    $data = new stdClass();
                    $data->id = $thisvalue->id;
                    $data->name = (string) $topics_choosen[$i];
                    $data->type = $type + 1;
                    $data->courseid = $COURSE->id;
                    $data->peerforumid = $peerforum->id;
            ...
            if ($type == 0) {
                foreach ($topics_choosen as $key => $value) {
                    $student_type_1 = $DB->get_records("peerforum_peergrade_users", array('peergradetype' => 1));
                    ...
                    if (empty($student_type_1)) {
                        ...
                        foreach ($student_type_1 as $i => $value) {
                            if ($student_type_1[$i]->topicsassigned == $topics_choosen[$key]) {
                    ...
                    if ($students_assigned < $students_to_assign) {
                        ...
                        $available_students = $DB->get_records("peerforum_peergrade_users", array('peergradetype' => 2));
                        ...
                        if (count($available_students) <= 9) {
                            ...
                            foreach ($rand_students as $j => $value) {
                                //$t = $rand_students[$key]->topicstoattribute;
                                //exlpode(";", $t);
                                //array_push($t, $topics_choosen[$key]);
                                //$updated_t = implode(";", $t);
                                $t = $topics_choosen[$key];

                                $data = new stdClass();
                                $data->id = $rand_students[$j];
                                $data->peergradetype = 1;
                                $data->topicsassigned = $t;
                                $DB->update_record("peerforum_peergrade_users", $data);
                            }
                        }
            ...
            $students_info = $DB->get_records("peerforum_peergrade_users", array('courseid' => $COURSE->id));
            //Get all discussion topics
            $discussiontopics = get_discussions_name($COURSE->id);

            $studentsavailable = count($students_info);

            //reset the database i guess..
            if (!empty($subjects_info)) {
                $DB->delete_records("peerforum_peergrade_subject", array('courseid' => $COURSE->id));
            }

            //Change all discussions' topic type
            foreach ($discussiontopics as $i => $value) {
                $data = new stdClass();
                $data->type = 1;
                $data->name = $discussiontopics[$i];
                $data->courseid = $COURSE->id;
                $data->peerforumid = $peerforum->id;
                $DB->insert_record("peerforum_peergrade_subject", $data);
            }
            ...
            //All topics type 2
            foreach ($subjects_info as $key => $value) {
                $data = new stdClass();
                $data->id = $subjects_info[$key]->id;
                $data->type = 2;

                $DB->update_record("peerforum_peergrade_subject", $data);
            }

            $students_type_1 = $DB->get_records("peerforum_peergrade_users", array('peergradetype' => 1));
    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                if ($peerforum->type == 'qanda' && !peerforum_get_user_posted_time($discussion->id, $userto->id) &&
                ...
                if (!isset($userto->canpost[$discussion->id])) {
                    ...
                    $userto->canpost[$discussion->id] =
                            peerforum_user_can_post($peerforum, $discussion, $userto, $cm, $course, $modcontext);
                }
                ...
                if ($post->parent) {  // This post is a reply, so add headers for threading (see MDL-22551)
                    $userfrom->customheaders[] =
                            'In-Reply-To: ' . peerforum_get_email_message_id($post->parent, $userto->id, $hostname);
                    $userfrom->customheaders[] =
                            'References: ' . peerforum_get_email_message_id($post->parent, $userto->id, $hostname);
                }
                ...
                $smallmessagestrings->peerforumname =
                        "$shortname: " . format_string($peerforum->name, true) . ": " . $discussion->name;
                $smallmessagestrings->message = $post->message;
                //make sure strings are in message recipients language
                $eventdata->smallmessage =
                        get_string_manager()->get_string('smallmessage', 'peerforum', $smallmessagestrings, $userto->lang);
        ...
        if ($digestposts_rs->valid()) {
            ...
            foreach ($userdiscussions as $userid => $thesediscussions) {
                ...
                $headerdata->userprefs =
                        '<a target="_blank" href="' . $headerdata->userprefs . '">' . get_string('digestmailprefs', 'peerforum') .
                        '</a>';
                ...
                foreach ($thesediscussions as $discussionid) {
                    ...
                    if ($discussion->name == $peerforum->name) {
                        ...
                        $posthtml .= " -> <a target=\"_blank\" href=\"$CFG->wwwroot/mod/peerforum/discuss.php?d=$discussion->id\">" .
                    ...
                    foreach ($postsarray as $postid) {
                        ...
                        if ($maildigest == 2) {
                            ...
                            $posttext .= "\n" . format_string($post->subject, true) . ' ' .
                                    get_string("bynameondate", "peerforum", $by);
                            $posttext .= "\n---------------------------------------------------------------------";

                            $by->name =
                                    "<a target=\"_blank\" href=\"$CFG->wwwroot/user/view.php?id=$userfrom->id&amp;course=$course->id\">$by->name</a>";
                            $posthtml .= '<div><a target="_blank" href="' . $CFG->wwwroot . '/mod/peerforum/discuss.php?d=' .
                                    $discussion->id . '#p' . $post->id . '">' . format_string($post->subject, true) . '</a> ' .
                                    get_string("bynameondate", "peerforum", $by) . '</div>';
...
function peerforum_make_mail_html($course, $cm, $peerforum, $discussion, $post, $userfrom, $userto) {
    ...
    $posthtml .= '<div class="navbar">' .
            ...
            '</a> &raquo; ' .
            '<a target="_blank" href="' . $CFG->wwwroot . '/mod/peerforum/index.php?id=' . $course->id . '">' . $strpeerforums .
            '</a> &raquo; ' .
            '<a target="_blank" href="' . $CFG->wwwroot . '/mod/peerforum/view.php?f=' . $peerforum->id . '">' .
            format_string($peerforum->name, true) . '</a>';
    if ($discussion->name == $peerforum->name) {
    ...
    $posthtml .= peerforum_make_mail_post($course, $cm, $peerforum, $discussion, $post, $userfrom, $userto, false, $canreply, true,
            false);
...
function peerforum_make_mail_post($course, $cm, $peerforum, $discussion, $post, $userfrom, $userto,
    ...
    if ($link) {
        ...
        $output .= '<a target="_blank" href="' . $CFG->wwwroot . '/mod/peerforum/discuss.php?d=' . $post->discussion . '#p' .
                $post->id . '">' .
...
function peerforum_print_post($post, $discussion, $peerforum, &$cm, $course, $ownpost = false, $reply = false, $link = false,
    ...
    if (!empty($post->ratingpeer)) {
        ...
        if (has_capability('mod/peerforum:viewallpeergrades', $PAGE->context) && $student) {
            $output .= html_writer::tag('br', ''); // Should produce <br />
        }
    }
...
function peerforum_shorten_post($message) {
    global $CFG;
    debugging('peerforum_shorten_post() is deprecated since Moodle 2.6. Please use shorten_text($message, $CFG->peerforum_shortpost) instead.',
            DEBUG_DEVELOPER);
...
function peerforum_get_optional_subscribed_peerforums() {
    ...
    $params = array_merge($courseparams,
            array('modulename' => 'peerforum', 'userid' => $USER->id, 'forcesubscribe' => PEERFORUM_FORCESUBSCRIBE));
 ...
 * @return string
 */
function peerforum_get_subscribe_link($peerforum, $context, $messages = array(), $cantaccessagroup = false, $fakelink = true,
        $backtoindex = false, $subscribed_peerforums = null) {
    ...
    if (peerforum_is_forcesubscribed($peerforum)) {
        ...
        if ($fakelink) {
            ...
            $PAGE->requires->js_function_call('peerforum_produce_subscribe_link',
                    array($peerforum->id, $backtoindexlink, $linktext, $linktitle));
            $link = "<noscript>";
        }
...
function peerforum_user_can_see_post($peerforum, $discussion, $post, $user = null, $cm = null) {
    ...
    $canviewdiscussion = !empty($cm->cache->caps['mod/peerforum:viewdiscussion']) ||
...
function peerforum_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $peerforumnode) {
    ...
    $cansubscribe = ($activeenrolled && $subscriptionmode != PEERFORUM_FORCESUBSCRIBE &&
            ($subscriptionmode != PEERFORUM_DISALLOWSUBSCRIBE || $canmanage));
    ...
    if ($enrolled &&
            peerforum_tp_can_track_peerforums($peerforumobject)) { // keep tracking info for users with suspended enrolments
...
function array_id_to_name($list) {
    global $DB, $COURSE;

    $names = array();
    $subject_names = $DB->get_records("peerforum_discussions");
...
function send_peergrade_notification($userto) {
    ...
    $userfrom = $DB->get_record("user", array('id' => 5));
 ...
 * @global object
 */
function apply_random_distribution($courseid) {
    global $DB;
    //Foreach topic get V students and assign them

    $students_info = $DB->get_records("peerforum_peergrade_users", array('courseid' => $courseid));
    //Get all discussion topics
    $discussiontopics = get_discussions_name($courseid);
