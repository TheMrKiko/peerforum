...
EOF;
    ...
    peerforum_grade_item_update($peerforum, $ratings, $peerforumgrades);
}
 ...
 */
function peerforum_grade_item_update($peerforum, $ratings = null, $peerforumgrades = null): void {
    global $CFG;
    require_once("{$CFG->libdir}/gradelib.php");
...
function peerforum_update_post($newpost, $mform, $unused = null) {
    ...
    if (isset($newpost->tags)) {
        core_tag_tag::set_item_tags('mod_peerforum', 'peerforum_posts', $post->id, $context, $newpost->tags);
    }
...
function peerforum_reset_gradebook($courseid, $type = '') {
    ...
    if ($peerforums = $DB->get_records_sql($sql, $params)) {
        foreach ($peerforums as $peerforum) {
            peerforum_grade_item_update($peerforum, 'reset', 'reset');
        }
    }
}
