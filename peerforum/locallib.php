...
/**
 * Library of functions for peerforum outside of the core api
 *
...
// Event types.
define('PEERFORUM_EVENT_TYPE_DUE', 'due');
...
class peerforum_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'peerforum');
        }
    }
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('peerforum_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'peerforum');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('peerforum_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'peerforum');
        }

        if (!$this->peerforum = $DB->get_record('peerforum', array('id' => $this->discussion->peerforum))) {
            throw new portfolio_caller_exception('invalidpeerforumid', 'peerforum');
        }

        if (!$this->cm = get_coursemodule_from_instance('peerforum', $this->peerforum->id)) {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_peerforum', 'attachment', $this->post->id, 'timemodified',
                        false);
                $embed =
                        $fs->get_area_files($this->modcontext->id, 'mod_peerforum', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = peerforum_get_all_discussion_posts($this->discussion->id, 'p.created ASC');
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach =
                        $fs->get_area_files($this->modcontext->id, 'mod_peerforum', 'attachment', $post->id, 'timemodified', false);
                $embed = $fs->get_area_files($this->modcontext->id, 'mod_peerforum', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/peerforum/discuss.php?d=' . $this->discussion->id;
    }
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
                ...
                'link' => $CFG->wwwroot . '/mod/peerforum/discuss.php?d=' . $this->discussion->id,
                'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) { // if we're writing leap, make the manifest to go along with the file
                ...
                return $this->exporter->write_new_file($leapwriter->to_xml(), $this->exporter->get('format')->manifest_name(),
                        true);
            }
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('peerforumdiscussion' . $this->discussionid,
                        get_string('discussion', 'peerforum') . ': ' . $this->discussion->name, 'selection');
     ...
     * that corresponds to a single peerforum post,
    ...
    private function prepare_post_leap2a(portfolio_format_leap2a_writer $leapwriter, $post, $posthtml) {
        $entry = new portfolio_format_leap2a_entry('peerforumpost' . $post->id, $post->subject, 'resource', $posthtml);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) &&
            ...
            $leapwriter->link_files($entry, $this->keyedfiles[$post->id], 'peerforumpost' . $post->id . 'attachment');
        }
    ...
    /**
     * this is a very cut down version of what is in peerforum_make_mail_post
     *
     * @param int $post
     * @return string
     * @global object
    ...
    private function prepare_post($post, $fileoutputextras = null) {
        ...
        $formattedtext =
                portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_peerforum', 'post', $post->id,
                        $format);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) &&
            ...
            $output .= '<br /><b>' . get_string('attachments', 'peerforum') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/peerforum:exportpost', $context)
                    || ($this->post->userid == $this->user->id
                            && has_capability('mod/peerforum:exportownpost', $context)));
        }
        return has_capability('mod/peerforum:exportdiscussion', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'peerforum');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class peerforum_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (($child = $this->browser->get_file_info($this->context, 'mod_peerforum', $this->filearea, $record->itemid))
...
/**
 * Returns peerforum posts tagged with a specified tag.
 *
 * This is a callback used by the tag area mod_peerforum/peerforum_posts to search for peerforum posts
 ...
 */
function mod_peerforum_get_tagged_posts($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = 1, $page = 0) {
    global $OUTPUT;
    $perpage = $exclusivemode ? 20 : 5;

    // Build the SQL query.
    $ctxselect = context_helper::get_preload_record_columns_sql('ctx');
    $query = "SELECT fp.id, fp.subject, fd.peerforum, fp.discussion, f.type, fd.timestart, fd.timeend, fd.groupid, fd.firstpost,
                    fp.parent, fp.userid,
                    cm.id AS cmid, c.id AS courseid, c.shortname, c.fullname, $ctxselect
                FROM {peerforum_posts} fp
                JOIN {peerforum_discussions} fd ON fp.discussion = fd.id
                JOIN {peerforum} f ON f.id = fd.peerforum
                JOIN {modules} m ON m.name='peerforum'
    ...
    $params = array('itemtype' => 'peerforum_posts', 'tagid' => $tag->id, 'component' => 'mod_peerforum',
    ...
    $builder = new core_tag_index_builder('mod_peerforum', 'peerforum_posts', $query, $params, $page * $perpage, $perpage + 1);
    while ($item = $builder->has_item_that_needs_access_check()) {
        ...
        $builder->walk(function($taggeditem) use ($courseid, $modinfo, $builder, $item) {
            // Checking permission for Q&A peerforums performs additional DB queries, do not do them in bulk.
            if ($taggeditem->courseid == $courseid && ($taggeditem->type != 'qanda' || $taggeditem->id == $item->id)) {
                $cm = $modinfo->get_cm($taggeditem->cmid);
                $peerforum = (object) ['id' => $taggeditem->peerforum,
                ...
                $accessible = peerforum_user_can_see_post($peerforum, $discussion, $post, null, $cm);
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $pageurl = new moodle_url('/mod/peerforum/discuss.php', array('d' => $item->discussion), 'p' . $item->id);
        ...
        return new core_tag\output\tagindex($tag, 'mod_peerforum', 'peerforum_posts', $content,
                $exclusivemode, $fromctx, $ctx, $rec, $page, $totalpages);
    }
}

/**
 * Update the calendar entries for this peerforum activity.
 *
 * @param stdClass $peerforum the row from the database table peerforum.
 * @param int $cmid The coursemodule id
 * @return bool
 */
function peerforum_update_calendar($peerforum, $cmid) {
    global $DB, $CFG;
    ...
    if (!empty($peerforum->duedate)) {
        $event->name = get_string('calendardue', 'peerforum', $peerforum->name);
        $event->description = format_module_intro('peerforum', $peerforum, $cmid, false);
        $event->format = FORMAT_HTML;
        $event->courseid = $peerforum->course;
        $event->modulename = 'peerforum';
        $event->instance = $peerforum->id;
        $event->type = CALENDAR_EVENT_TYPE_ACTION;
        $event->eventtype = PEERFORUM_EVENT_TYPE_DUE;
        $event->timestart = $peerforum->duedate;
        $event->timesort = $peerforum->duedate;
        $event->visible = instance_is_visible('peerforum', $peerforum);
    }

    $event->id = $DB->get_field('event', 'id',
            array('modulename' => 'peerforum', 'instance' => $peerforum->id, 'eventtype' => PEERFORUM_EVENT_TYPE_DUE));
    ...
    } else if (!empty($peerforum->duedate)) {
