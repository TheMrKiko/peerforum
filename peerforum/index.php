...
add_to_log($course->id, 'peerforum', 'view peerforums', "index.php?id=$course->id");
...
$subscribed_peerforums = peerforum_get_subscribed_peerforums($course);
    ...
    foreach ($modinfo->get_instances_of('peerforum') as $peerforumid => $cm) {
        ...
        if (!peerforum_is_forcesubscribed($peerforum)) {
            ...
            if ((has_capability('moodle/course:manageactivities', $coursecontext, $USER->id) ||
                            $peerforum->forcesubscribe != PEERFORUM_DISALLOWSUBSCRIBE) && $subscribe && !$subscribed && $cansub) {
                peerforum_subscribe($USER->id, $peerforumid);
            } else if (!$subscribe && $subscribed) {
                peerforum_unsubscribe($USER->id, $peerforumid);
            }
        }
    }
    $returnto = peerforum_go_back_to("index.php?id=$course->id");
    $shortname = format_string($course->shortname, true, array('context' => context_course::instance($course->id)));
    if ($subscribe) {
        add_to_log($course->id, 'peerforum', 'subscribeall', "index.php?id=$course->id", $course->id);
        redirect($returnto, get_string('nowallsubscribed', 'peerforum', $shortname), 1);
    } else {
        add_to_log($course->id, 'peerforum', 'unsubscribeall', "index.php?id=$course->id", $course->id);
    ...
    if ($learningpeerforums) {
        ...
        foreach ($learningpeerforums as $peerforum) {
            ...
            if ($can_subscribe) {
                $row[] = peerforum_get_subscribe_link($peerforum, $context, array('subscribed' => $stryes,
                        'unsubscribed' => $strno, 'forcesubscribed' => $stryes,
                        'cantsubscribe' => '-'), false, false, true, $subscribed_peerforums);
...
if (!isguestuser() && isloggedin() && $can_subscribe) {
    ...
    echo html_writer::tag('div',
            html_writer::link(new moodle_url('/mod/peerforum/index.php',
