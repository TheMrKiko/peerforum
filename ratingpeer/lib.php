...
class ratingpeer_manager {
    ...
    /**
     * Returns an array of ratingpeers for a given item (forum post, glossary entry etc). This returns all users ratingpeers for a
     * single item
     *
     * @param stdClass $options {
    ...
    public function get_ratingpeers($options) {
        ...
        list($itemtablename, $itemidcol, $itemuseridcol) =
                plugin_callback($type, $name, 'ratingpeer', 'get_item_fields', array($options), $default);
        ...
        foreach ($options->items as $item) {
            ...
            foreach ($userratingpeers as $userratingpeer) {
                ...
                if ($item->{$itemidcol} == $userratingpeer->itemid) {
                    ...
                    $ratingpeeroptions->ratingpeer =
                            min($userratingpeer->usersratingpeer, $ratingpeeroptions->settings->scale->max);
    ...
    protected function generatepeer_ratingpeer_settings_object($options) {
        ...
        $pluginpermissionsarray =
                $this->get_plugin_permissions_array($options->context->id, $options->component, $options->ratingpeerarea);
    ...
    public function get_plugin_permissions_array($contextid, $component, $ratingpeerarea) {
        ...
        $defaultpluginpermissions =
                array('ratepeer' => false, 'view' => false, 'viewany' => false, 'viewall' => false);//deny by default
        if (!empty($component)) {
            list($type, $name) = core_component::normalize_component($component);
            $pluginpermissionsarray =
                    plugin_callback($type, $name, 'ratingpeer', 'permissions', array($contextid, $component, $ratingpeerarea),
                            $defaultpluginpermissions);
        } else {
...
/**
 * The ratingpeer_exception class provides the ability to generatepeer exceptions that can be easily identified as coming from the
 * ratingpeers system
 *
 * @package   core_ratingpeer
 * @category  ratingpeer
